...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, and Kimi quota windows, and it must never route, recommend, proxy, intercept, log in, or import browser cookies. It must not mutate provider state, with one narrowly scoped exception: Claude short-lived-credential renewal (see the Claude OAuth refresh contract below). Renewal is credential rotation, not a login — no browser, authorization-code exchange, or new consent — and it is the only credential mutation quota-axi performs; no other provider's credentials are ever written.
...
- Claude OAuth refresh lives in `src/providers/claude-oauth-refresh.ts`: on a definitive usage 401/403 with a refresh token present, `fetchQuota` performs at most one `refresh_token` grant (authoritative Claude Code login-flow contract — token endpoint, public client id, and `claudeAiOauth` schema derived only from the shipped CLI's own OAuth behavior) and retries usage at most once. It writes the renewed short-lived credential back atomically to the same source and context (`0600` file or the exact pinned Keychain account/service), preserves refresh-token rotation, and adopts a concurrently rotated stored credential instead of clobbering it. Only HTTP 401/403 or an `invalid_grant` error code is a rejected refresh token (definitive sign-in); every other token-endpoint failure is transient (session preserved, stale-eligible), and after one transient refresh failure no later definitive usage rejection in that read may report sign-in required. Write-back is best-effort and time-bounded, and its outcome is surfaced as an `oauth-refresh-write-back` attempt so a lost rotation is never silent. No token or raw token-endpoint response is ever logged, cached, embedded, hashed, or written anywhere but the credential's own source; the one inherent exposure is the renewed blob in the `security add-generic-password -U` argv during macOS Keychain write-back (readable by same-user processes for that one call; `security` has no stdin path for it). The full behavior contract is in [README Provider notes](README.md#provider-notes).
