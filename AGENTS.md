...
- `coverage` is a static provider-source catalog: it makes no provider calls or credential reads, separates billing mode from implementation status, and points supported non-metered sources to their live quota command without inventing unavailable allowance.
...
- Grok reads `$GROK_AUTH_JSON`, inline `$GROK_AUTH`, `$GROK_AUTH_PATH`, or `$GROK_HOME/auth.json`/`~/.grok/auth.json`, selects session-scoped auth instead of API-key entries, and recognizes OIDC records scoped to `auth.x.ai` with `auth_mode` or `authMode` set to `oidc`.
- Grok falls back to the Pi coding agent's shared credential store (`$PI_AUTH_JSON`, `$PI_HOME/agent/auth.json`, `~/.pi/agent/auth.json`), using only its `xai` entry and only when that entry is an `oauth` session. Pi writes `expires` as an epoch number, not an ISO string. Every discovered source is listed by `auth` (path and status only), so tests must sandbox `PI_HOME` or they will read a developer's real session.
- Grok quota comes from the read-only consumer `grok_api_v2.GrokBuildBilling.GetGrokCreditsConfig` operation and can include the shared `credits` window, product-scoped windows, reset, and prepaid balance. Only for this exact operation, a valid weekly/monthly period makes omitted usage floats official proto3 zero; never derive quota from monetary fields.
- Grok separates subscription allowance from metered xAI API usage via `QuotaWindow.lane` (`subscription` / `metered`); the two are never interchangeable for dispatch. Consumer quota windows are subscription-lane windows, while prepaid balance is metered money and a zero prepaid balance is omitted so dormant metering cannot read as an exhausted subscription.
- An xAI API key yields `state.status: unsupported` (authenticated but unable to answer for subscription quota) rather than `auth_required`, and is never spent as a probe; `unsupported` is not clearable by re-authenticating.
- Grok cache fallback accepts only current `web` source snapshots, not legacy `api` billing-proxy entries. Never send cookies, launch Grok, refresh its credentials, or retain raw responses.
...
- JSON report shape and quota interpretation are documented in [README Output Model](README.md#output-model). `state.retryAfter` can appear for provider rate limits, and `state.reason: keychain_access_required` plus `state.remedyCommand` can appear when a stale or unavailable Claude result is blocked by a skipped macOS Keychain prompt.
- Endpoint throttling and exhausted quota are distinct seat states and must never be conflated for dispatch: an HTTP 429 on the usage endpoint yields `state.status: rate_limited` with `state.retryAfter` and no windows (capacity unknown, retry later), while an exhausted subscription is a successful `fresh` read carrying windows at `percentUsed` 100 / `percentRemaining` 0 with `resetsAt` (capacity known-spent, no `retryAfter`). One seat's 429 stays a bounded per-seat row and never reads as all-Claude-unavailable or as spent quota; locked by `test/providers/claude.test.ts` and the throttle-vs-exhausted case in `test/cli.test.ts`.
