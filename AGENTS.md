...
- Kimi resolves only Pi's `kimi-coding` API-key credential through the supported Pi credential API, then sends one bounded, redirect-disabled `GET` to `https://api.kimi.com/coding/v1/usages`. It reports principal `weekly`, the first exact 18,000-second limit as `five_hour`, and other valid limits as unknown windows; never read Pi auth directly, honor custom transport data, launch Pi/Kimi, refresh credentials, or expose identity or raw payloads.
