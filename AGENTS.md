...
- TokenRouter reads the management wallet from `TOKENROUTER_MGMT_KEY` and calls the read-only `/api/management/self/wallet` endpoint. It reports USD balance plus a credit-pool usage window; it never selects models or routes requests.
