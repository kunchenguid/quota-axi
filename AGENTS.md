...
- Grok quota comes from the read-only consumer `grok_api_v2.GrokBuildBilling.GetGrokCreditsConfig` operation and can include the shared `credits` window, product-scoped windows, reset, and prepaid balance. Only for this exact operation, a valid weekly/monthly period makes omitted usage floats official proto3 zero; never derive quota from monetary fields. Cross-provider duration provenance and Grok product-ID stability are documented in [README Window duration provenance](README.md#window-duration-provenance) and [README Provider windows](README.md#provider-windows).
