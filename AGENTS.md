...
- It reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, and Z.AI quota windows, and it must never route, recommend, proxy, intercept, log in, import browser cookies, or mutate provider state.
...
- Cursor has two independent, additive credential sources: the editor `state-vscdb` store and, on macOS, the Cursor CLI (`cursor-agent`) `cli-keychain` source in `src/providers/cursor-cli-credential.ts`. Either source alone supports auth inspection and quota refresh, so Cursor Desktop is not required. Keep the editor source first, preserve the existing Keychain prompt gate, never read `cursor-refresh-token`, and do not invent credential refresh. Attempt naming, credential locations, and recovery behavior are documented in [README Provider notes](README.md#provider-notes).
...
- Z.AI reads Pi's `zai` entry (literal API key or unexpired OAuth access token) with the same in-place contract as `src/providers/pi-kimi-credential.ts`, then reads the first-party `GET https://api.z.ai/api/monitor/usage/quota/limit` endpoint. That endpoint has no published REST contract, so `src/providers/zai.ts` identifies every window by its self-described `(type, unit, number)` triple - never by array position - and anything it cannot identify stays an `unknown` window in `state.untrustedWindowIds` instead of being folded into a bound. Window mapping, degradation, and cache posture are documented in [README Provider windows](README.md#provider-windows) and [README Provider notes](README.md#provider-notes).
