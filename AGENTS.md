...
- Cursor reads `$CURSOR_STATE_DB` or the local Cursor state database via `sqlite3 -readonly` for `cursorAuth` values, then falls back to the Cursor CLI `$CURSOR_CLI_AUTH_JSON` or `$XDG_CONFIG_HOME/cursor/auth.json` (`cursor-cli-auth`), and can report `included_usage`, `auto_usage`, `api_usage`, and optional `spend_limit` windows from the first-party dashboard usage endpoint. Editor-store precedence, CLI JWT-expiry handling, and the sanitized `x-cursor-*` CLI headers are documented in [README Security Posture](README.md#security-posture).
- If `sqlite3` is unavailable, the `state-vscdb` source is reported as skipped with `sqlite3_unavailable` (the CLI sources still run); do not treat that as permission to install system packages.
- Cursor has three independent, additive credential sources: the editor `state-vscdb` store, the Cursor CLI `cursor-cli-auth` auth.json source in `src/providers/cursor-cli-auth.ts`, and, on macOS, the Cursor CLI (`cursor-agent`) `cli-keychain` source in `src/providers/cursor-cli-credential.ts` (identity from `cli-config.json`, access token from the `cursor-access-token`/`cursor-user` Keychain item behind the same `--allow-keychain-prompt` gate and access-marker pattern Claude uses). Any source alone supports auth inspection and quota refresh, so Cursor Desktop is not required; the non-prompting stores are tried first and the prompt-gated Keychain last. quota-axi never reads `cursor-refresh-token` and never refreshes Cursor credentials; CLI access-token refresh is an intentional, documented gap (expired token => `Cursor sign-in required`, remedy `cursor-agent login`), so do not add a guessed refresh endpoint. Attempt naming, credential locations, and recovery behavior are documented in [README Provider notes](README.md#provider-notes).
- GitHub Copilot reads `$GITHUB_COPILOT_APPS_JSON` or the local `github-copilot/apps.json` auth file, then falls back to the GitHub Copilot CLI `$GITHUB_COPILOT_CLI_CONFIG` or `~/.copilot/config.json` `.copilotTokens` (`copilot-cli-config`, comment-line-tolerant), and can report quota snapshot windows such as `chat`, `completions`, and `premium_interactions` from GitHub's first-party Copilot user endpoint. If the endpoint only exposes entitlement and no numeric quota windows, return a fresh provider report with `windows: []` rather than inventing percentages.
- Copilot only sends tokens associated with public GitHub hosts to the public endpoint; host-specific GitHub Enterprise tokens are treated as unavailable there, from both credential sources.
