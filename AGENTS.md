...
- The uniform credential-override contract (schema-1 envelope on stdin or a 0600 `$QUOTA_AXI_CREDENTIALS_FILE`) is specified in [docs/credential-override.md](docs/credential-override.md) and parsed in `src/credential-override.ts`; flights run through `runOverrideFlight` in `src/providers/common.ts`. When an override entry exists for a provider: only that credential is used, no local source or vendor CLI fallback runs, no stale-cache answer is served, override snapshots are never cached, success reports `source: "override"`, and 401/403 surfaces as `auth_required` + `override_rejected`. Absent an envelope, behavior is byte-for-byte unchanged. Token bytes never reach argv, any environment block, the cache, logs, or output; `auth --json` advertises `capabilities.credentialOverride` for consumer feature detection.
