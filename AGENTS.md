...
- `quota` is the implicit default command: `runAxiCli` routes on `argv[0]` and rejects a leading flag, so `src/cli.ts` `normalizeArgv` prepends `quota` for a bare call or flag-first call (`quota-axi --json`) while leaving `auth`, `update`, and the single-token `--help`/version through to the SDK. Validation errors throw `AxiError("...", "VALIDATION_ERROR")` (exit 2); the all-rows-failed path sets `process.exitCode = 1` and still renders. Every quota response carries a top-level `summary` (`availability` of `ok`/`partial`/`unavailable` plus `ok`/`unavailable`/`total` row counts, one row per provider or Claude seat) computed in `src/advice.ts`; `commands.ts` derives the exit code from `summary.availability === "unavailable"` so a single seat's failure never reads as complete unavailability. Full and partial availability both exit 0.
...
- `--allow-keychain-prompt` is the first-time opt-in that permits the Claude Keychain value read which can prompt; prompt-capable multi-seat reads are serialized, and agents should relay the profile-preserving `remedyCommand` plus one-time "Always Allow" grant when `keychain_access_required` advice appears.
