...
- Kimi prefers a literal Pi-managed `kimi-coding` API key from a bounded, read-only parse of `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`), then falls back only when that credential is unavailable to a fresh official Kimi Code CLI `access_token` at `$KIMI_CODE_HOME/credentials/kimi-code.json` (default `$HOME/.kimi-code/credentials/kimi-code.json`). It sends one bounded, redirect-disabled `GET` to `https://api.kimi.com/coding/v1/usages` and reports principal `weekly`, the first exact 18,000-second limit as `five_hour`, and other valid limits as unknown windows. Never resolve environment or command references, use an ambient API-key source, use `refresh_token`, honor custom transport data, launch Pi/Kimi, refresh or write credentials, create a device ID, or expose identity or raw payloads.
