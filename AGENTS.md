...
- Antigravity is explicit-selection-only. With `--provider antigravity`, it runs only the documented 1.1.11+ `agy -p "/usage" --output-format json` print command with the real home/minimal current-user session for the CLI's existing system-keyring auth, plus a temporary cwd and XDG config/cache/state directories; auth inspection checks executable availability only. quota-axi never reads or copies Antigravity credentials, keyrings, or browser cookies and never refreshes auth, calls a private RPC, starts a model turn, or launches a second command. Live `weekly` and `5h` buckets receive trusted durations even when groups omit `id`; the vendor's signed-out browser fallback is a disclosed draft policy question for the maintainer.
