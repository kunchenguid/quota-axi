...
- Grok quota comes from the read-only consumer `grok_api_v2.GrokBuildBilling.GetGrokCreditsConfig` operation and can include the shared `credits` window, product-scoped windows, reset, and prepaid balance. Only for this exact operation, a valid weekly/monthly period makes omitted usage floats official proto3 zero; never derive quota from monetary fields. Top-level `credits.remaining` is prepaid balance, distinct from the period `windows` credits percentage.
- An expired Grok access token with a remaining refreshable session reports `Grok access token expired` and `state.reason: credentials_expired` (remedy: open the Grok CLI once). Reserve `Grok sign-in required` for missing/unusable session auth without a refreshable record and for live provider auth rejection. Never launch Grok or refresh credentials from quota-axi.
