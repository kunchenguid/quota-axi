...
A credential value leaves the process only as the bearer of the first-party request it authenticates, and is never printed, logged, cached, or written into a test fixture. Meta Muse Spark has one readings-only exception to the exchange boundary: its identity bearer may call Meta's key endpoint to read the subscription-usage snapshot returned alongside a Model API key, provided that key is immediately discarded and is never printed, logged, cached, persisted, or used.
...
A change should be resisted when it publishes a number no provider supports, holds a boundary at the cost of a false report, spends the quota it is measuring, mints or rotates a credential quota-axi does not own outside the single Meta Muse readings-only exception above, or grows the surface into a product this is not.
