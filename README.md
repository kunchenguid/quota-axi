...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Fireworks AI, and Antigravity (`agy`) quota windows in one [AXI](https://axi.md)-shaped call.
...
auth[19]{provider,source,path,status,error}:
  ...
  fireworks,env,$FIREWORKS_API_KEY,available,none
  fireworks,fireworks:auth.ini,~/.fireworks/auth.ini,missing,none
help[1]:
...
| Flag                                                                                     | Description                                                               |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,fireworks` | Scope providers                                                           |
...
Fireworks reports independent per-resource account quotas - the vendor documents GPU quota per accelerator type and placement, the account request-rate ceiling, and the monthly spend limit - and its quota list carries no cycle for any of them. quota-axi has no evidence of how they jointly bound a common scope, so each quota is reported as its own `unknown`-kind window with `percentUsed` computed from its own `usage` over its enforced `value` (not the approved `maxValue`; numbers and proto-JSON numeric strings are accepted), the provider's semantics stay `unknown` with every window named in `unresolvedWindowIds`, and no effective remaining percentage, pace, runway, or selection is claimed. A quota whose usage cannot be expressed as a ratio of its own enforced value bounds nothing, so it is named in `state.untrustedWindowIds` rather than becoming a window with an invented percentage.
...
| Fireworks AI           | Reports one `quota:<id>` window per measurable quota (for example `quota:h100-us-iowa-1`). See [Quota windows](#quota-windows) for field provenance, unmeasurable entries, and interpretation. An account that declares no quotas is a fresh report with an empty `windows` list rather than invented percentages.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| Fireworks AI   | `FIREWORKS_API_KEY` (optionally paired with `FIREWORKS_ACCOUNT_ID`) when it is set to a usable literal key, then only the `api_key` and `account_id` entries of the `firectl` login file at `$FIREWORKS_AUTH_INI` when set, otherwise `~/.fireworks/auth.ini`                                                                                                                                                                                                                |
...
- Setup: export `FIREWORKS_API_KEY` with a key from the Fireworks console, or sign in with the vendor's own CLI (`firectl set-api-key <KEY>` or `firectl signin`), which writes `~/.fireworks/auth.ini`. Either one alone is enough; quota-axi checks the environment first because the vendor's own SDKs resolve `FIREWORKS_API_KEY` before opening any store, so a key there names the account a live session is actually using. A blank value in either store selects nothing and leaves the other exactly as it was.

  ```sh
  export FIREWORKS_API_KEY="$(pass show fireworks/api-key)"   # never paste the key into a command line
  quota-axi --provider fireworks
  ```

- The account id is the path segment of the quota URL, so it must be resolved before any quota request. quota-axi takes it from the same store the key came from - `FIREWORKS_ACCOUNT_ID` for the environment key, `account_id` for the `auth.ini` login - and never pairs one store's key with another store's account. When a store supplies a key but no account id, it resolves the id from the vendor's own read-only account listing and uses it only when the listing is complete and every entry parses to a valid account id, yielding exactly one distinct id. A non-empty `nextPageToken`, a declared `totalSize` that does not parse to 1, an invalid entry, or an empty or ambiguous listing is reported as `fireworks_account_unresolved` rather than guessed, because reading the wrong account would report another account's numbers as this one's. Set `FIREWORKS_ACCOUNT_ID` to name the account explicitly. An account id that is not a bare path segment is rejected before it can reach a URL.
- `auth.ini` also holds SSO material (`id_token`, `refresh_token`, `client_id`, `cognito_domain`, `issuer_url`). quota-axi's reader is a narrow whitelist walker that keeps only `api_key` and `account_id`, discarding other fields from the parsed result. The reader enforces a 64 KiB byte limit before strict UTF-8 decoding; oversized files and invalid UTF-8 are read failures, not parsed configuration. The raw file text is temporarily in memory during parsing. It reads both the vendor's bare `key = value` layout and a section-headed one. Environment, template, and command references are not resolved or executed, so a key that holds one is reported as an `env` or `fireworks:auth.ini` source problem rather than sent as a header value. Key values are never logged, rendered, or cached. `FIREWORKS_AUTH_INI` overrides the login file path.
- For each credential probed, it sends one redirect-disabled `GET` to `https://api.fireworks.ai/v1/accounts/<account>/quotas` with the key as a `Bearer` token and a 15 second deadline, plus at most one `GET https://api.fireworks.ai/v1/accounts` when the account id had to be discovered. A quota list that cannot be proven whole fails the read with `fireworks_quota_incomplete`: a non-empty `nextPageToken`, or a declared `totalSize` that exceeds the rows actually returned or cannot be read as a number. Pages are not followed or published as a partial reading, because a short list is indistinguishable from a complete one in the output. A non-object quota response fails with `schema_invalid`. It never writes credentials or changes vendor state, never calls the quota update endpoint, and never touches billing, deployment, or API-key management operations.
- See [Quota windows](#quota-windows) for interpretation. A GPU quota's percentage is concurrent capacity in use, not spend, and none of these rows is a token or credit balance.
- HTTP 401 is a definitive rejection of that key and falls through to the next store; when every readable key is rejected and no store read failed, the report is `auth_required`. An unreadable or oversized login file leaves the result indeterminate (`state.status: error`, `credential_resolution_failed`) when no source produces a reading, rather than reporting a sign-out. HTTP 403 is treated differently: the key was accepted and the quota operation refused, so quota-axi reports `state.status: unavailable` with `authStatus: usable` and no windows, never a sign-out - a console key without quota-read permission is not a logged-out account. A permission-refused key ends the reading without falling through to another store: that store may name a different account, and quota-axi will not answer for an account the caller did not ask about. Transport, timeout, rate-limit, redirect, decoding, and server failures stop without credential switching and never become authentication verdicts.
- A Fireworks API key may carry a vendor-side `expireTime` chosen when the key was created; a key created without one never expires. Neither store quota-axi reads records that timestamp - the environment variable is a bare key and the `firectl` login file keeps only the key value - and quota-axi never calls the key-management endpoints that would reveal it, so it has no local expiry to order candidates by and never infers one. There is no refresh token and no vendor-owned non-interactive rotation command either, so Fireworks has no delegated refresh: an expired key is established only when the quota endpoint itself rejects it with HTTP 401. See [Cache](#cache) for the Fireworks fallback policy.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Fireworks AI, Antigravity | none                              | read-only; see the per-provider notes below           |
...
Providers with no established non-interactive rotation command stay read-only on purpose. That is a documented limitation rather than a reason to force an unsafe path: Cursor's CLI token is long-lived and no non-interactive `cursor-agent` command was observed to rotate it, GitHub Copilot's stored OAuth tokens (in `apps.json` and the GitHub CLI's `hosts.yml`) do not expire, Z.AI uses a non-expiring API key, Alibaba is accessed through the read-only `bl` usage command, OpenCode Go has no vendor-owned rotation command, Fireworks API keys expose no local expiry metadata and have no vendor-owned rotation command, Pi-owned OAuth entries (`openai-codex`, `xai`, `kimi-coding`) have no non-interactive Pi refresh command, and Antigravity exposes no credential store at all.

### Safety guarantees

- Quota and auth HTTP requests go only to first-party provider usage, quota, billing, entitlement, read-only account-discovery, or read-only credential-liveness endpoints with the user's local credentials; quota-axi's direct Antigravity requests stay on 127.0.0.1 loopback.
...
| Fireworks stale fallback               | None. A Fireworks snapshot cannot prove which account it describes, so a superseded reading is never served for an account it may not describe.                                                                                                                                                                                                                                                                                                                                                                                                                              |
