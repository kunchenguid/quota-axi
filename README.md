...
| Kiro                   | Reports the plan's single `credit` window from the first-party `GetUsageLimits` response as the `all_models` bound, plus the current plan, credits remaining, and provider reset time. The response reports the next reset but no cycle start or duration, so pace, runway, and selection stay unknown. It reads the local CLI token store without modifying it; use `KIRO_CLI_DB`, `KIRO_REGION`, and optional `KIRO_PROFILE_ARN` for non-default installations. |
...
- It opens the current user's Kiro CLI SQLite database read-only and reads only the `kirocli:odic:token` record. `KIRO_CLI_DB` overrides the default path; `KIRO_REGION` overrides the stored region; and optional `KIRO_PROFILE_ARN` is included only when configured. An absent store is `missing` and is never opened; a file that is not a database, a store without the `auth_kv` table, a malformed token record, or one without an `access_token` is `invalid` (`credentials_invalid`, a present credential that does not work); a locked or busy store is a transient `error` that preserves the cache rather than a sign-out.
...
- Definitive credential failures (a missing token record after `kiro-cli logout`, an invalid store, or a first-party 401/403 that is not soft expiry) retire the matching Kiro cache on a best-effort basis. Failed reads never reuse cached plan or credits: the current credential reader supplies no verified non-secret account-continuity evidence, and an unchanged database, region, and profile can hold a different account between reads. Transient failures preserve the cache on disk and report the current error, including retry guidance when available.
- It never refreshes, writes, or deletes Kiro credentials; it never launches `kiro-cli`; and token, email, profile ARN, and refresh-token values are excluded from reports, cache, errors, and tests.
