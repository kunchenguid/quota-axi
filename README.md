...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), and Command Code quota windows in one [AXI](https://axi.md)-shaped call.
...
- `--allow-claude-inference` is the explicit numeric fallback for that specific env-token scope boundary. It launches the installed native Claude Code client once from a new empty temporary directory with safe mode, tools, MCP/browser integration, permission prompts, slash commands, and session persistence disabled; the fixed prompt asks for exactly `OK`, caps output budget, and may spend normal startup plus inference tokens. SDK debug output is bounded in memory, never persisted or rendered, and reduced to validated unified five-hour/seven-day utilization and reset headers. The report uses source `cli`; `--full` records a `claude-native-inference` attempt. Unknown debug formats, missing headers, unavailable/incompatible Claude, timeouts, and process failures report unavailable rather than inventing quota. A native 429 reports `rate_limited` with its bounded Retry-After and keeps any validated unified five-hour/seven-day windows that response itself carried in `windows`, and that observation wins even when the client then retries past quota-axi's execution or output bound; a bound with no observed rate limit stays a timeout or output-limit failure. Because the env token is consulted first and that scope denial ends discovery, a skipped Keychain candidate on such a reading never produces `keychain_access_required` advice, whether the denial or a failed native run is the final error. A nonblank `ANTHROPIC_API_KEY` in the environment also reports unavailable (`claude_native_api_key_present`) before anything is launched, because the native client could bill that key instead of measuring the selected OAuth session; quota-axi checks presence only and never reads, logs, or unsets it. Claude may still perform its normal transient startup cache/log writes. The flag is off by default, is rejected by `auth`, `models`, and `--profile-only`, and requires `--once` with `--tui` so it cannot become a recurring paid probe.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity, Command Code | none                              | read-only; see the per-provider notes below           |
