...
- One conclusive listing is performed per account per process, so a long-lived `--tui` refresh loop does not repeat it; an inconclusive listing is retried on the next read. A suffix that rotates after a conclusive listing is picked up by the next run. The same applies to a conclusive absence: if the listing found no Claude item, signing in while the process is still running requires restarting it to be detected.
