...
Claude credential failures without a usable access token preserve the precise `credentials_missing` or `credentials_invalid` error. A usage response with HTTP 401 reports `Claude sign-in required`. This definitive failure returns no windows and retires the Claude cache instead of masking current authentication state with stale quota. The exception is a withheld macOS Keychain value (`keychain_access_denied`): a 401 from a non-keychain candidate, such as an expired oauth-file sidecar, is not treated as signed-out and does not retire the cache. The report surfaces `keychain_access_denied` and keeps any still-valid stale snapshot. HTTP 403 is not sufficient authentication evidence because network policy and WAF denials use it too; it stays unavailable or stale rather than claiming sign-out.
