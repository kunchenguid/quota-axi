...
- For the default profile, quota-axi lists Keychain metadata with `security dump-keychain <default keychain>` (no values, raw data, or ACLs), bounded to the login/default keychain reported by `security default-keychain`, and considers only generic-password items named `Claude Code-credentials` or `Claude Code-credentials-<8 lowercase hex>`. It selects the most recently modified matching item; unknown modification times sort last, and ties use service then Keychain path in ascending lexical order. The listing is bounded to five seconds and 16 MiB, and each credential pass reads at most one value, pinned to its listed Keychain as well as service and account. Older accumulated items are not retried. A record the parser cannot read never discards an item that was found; it only withholds the absence verdict.
- Claude Code's suffix is opaque, so quota-axi does not assume how it is derived and cannot tell one profile's item from another's by name. Default-profile enumeration therefore selects the most recently modified item matching the accepted names for the current account, which may be an item Claude Code wrote for a different `CLAUDE_CONFIG_DIR` profile if that profile was the most recent login. The resulting windows and `--full` account identity are that item's, while the snapshot is cached under the default profile's configuration context. Set `CLAUDE_CONFIG_DIR` to read a specific profile exactly.
- A located item is listed once per account per process, so a long-lived `--tui` refresh loop does not repeat the listing; a suffix that rotates afterwards is picked up by the next run. A listing that located nothing is repeated on every read, so signing in while a refresh loop is running is detected on its next tick.
- When the keychain cannot be listed at all, quota-axi falls back to the exact `Claude Code-credentials` presence or value read that predates suffix discovery, and reports the check failure rather than absence, because an unlistable keychain is not evidence about the item.
- A nonempty `CLAUDE_CONFIG_DIR` retains the existing exact `Claude Code-credentials-<sha256(NFC config path).slice(0,8)>` lookup. It never enumerates, so it never falls through to another profile's item; default-profile enumeration does not guess Claude Code's current suffix algorithm.
...
- A generic-password item for the same account whose service starts with `Claude Code-credentials` but does not match the accepted names is never opened as a credential, and it withholds the absence verdict: the reading falls back to the exact unsuffixed read and reports a Keychain failure rather than a sign-out, so the cache survives a naming scheme quota-axi has not seen.
