...
auth[17]{provider,source,path,status,error}:
  ...
  zai,env:ZAI_API_KEY,ZAI_API_KEY,missing,none
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `gh:hosts.yml`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `env:ZAI_API_KEY`, `pi:zai`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
| Z.AI           | `ZAI_API_KEY` when it is set to a usable literal key, then Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) for a literal Coding Plan `api_key` entry under `zai`, then opencode's `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `~/.local/share/opencode/auth.json`) for a literal key under `zai-coding-plan`, `zai`, `z-ai`, `z.ai`, `zhipu`, or `zhipuai`                                                                  |
...
- It checks three independent sources for a literal Coding Plan API key: the `ZAI_API_KEY` environment variable first, then Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`), then opencode's `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `~/.local/share/opencode/auth.json`; `%LOCALAPPDATA%\opencode\auth.json` on Windows). Z.AI is locally usable when any source is usable. A successful environment-key probe reports source `env:ZAI_API_KEY`; a successful Pi-backed probe reports source `pi:zai`; a successful opencode probe reports `opencode:auth.json`.
...
- In opencode's store, the `zai-coding-plan`, `zai`, `z-ai`, and `z.ai` ids resolve to `api.z.ai`, and `zhipu` / `zhipuai` resolve to `open.bigmodel.cn`.
- `ZAI_API_KEY` is preferred over both stores, because the Z.AI SDK and vendor coding plugins read it first and it is the only source when a secret manager injects the key at run time. Absent, empty, and whitespace-only values select nothing; a non-blank value that is not a usable literal key is reported `invalid` rather than sent. The key is used for the request only and is never logged, cached, or written back. The environment source always targets `api.z.ai`, so a mainland Zhipu `open.bigmodel.cn` key exported there gets a 401 and the reading falls through to the stored sources. Because an environment key can name a different account from the stored credential, Z.AI snapshots carry an opaque context identifier for the source that actually answered, published the way Kimi publishes its own: a usable `ZAI_API_KEY` is identified by a domain-separated one-way SHA-256 digest of the key, and a stored credential by its source name, so `pi:zai` and `opencode:auth.json` stay apart from each other as well as from the environment. A snapshot cached under one account is never served as stale for another, while the same account keeps reusing its own. The identity is never derived from the environment at write time: when a rejected `ZAI_API_KEY` hands over to a stored credential, the reading is filed under the credential that answered, not the key that failed. A key the vendor definitively rejects has its own cached snapshot retired and is never served stale numbers for the rest of that run, and that retirement is scoped to it — a rejected key does not discard the snapshots of accounts the run never contacted. Identifying the environment key by its digest is a deliberate departure from not deriving cache context from secret-bearing bytes (the Kimi `config.toml` rule): a bare environment variable carries no other account identity, and accurate per-account reporting requires one. Only the opaque identifier is persisted; the key is never stored or rendered.
