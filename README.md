...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), Command Code, MiniMax, MiMo, DeepSeek, OpenRouter, and ElevenLabs quota windows in one [AXI](https://axi.md)-shaped call.
...
- **Host network policy** - Claude, Codex, Copilot, Cursor, Grok, Z.AI, Command Code, MiniMax, DeepSeek, OpenRouter, and ElevenLabs' outbound HTTP calls honor standard `HTTP_PROXY`, `HTTPS_PROXY`, and `NO_PROXY` environment variables (including lowercase forms). This only follows the user's configured egress path; quota-axi does not expose a proxy service or print proxy URLs.
- **Opt-in secure-store access** - macOS Claude, Cursor CLI, and Copilot CLI Keychain value reads, plus Windows Copilot Credential Manager reads, are skipped on plain calls until `--allow-keychain-prompt` succeeds once for that source, then future quota calls reuse the corresponding grant.
...
- A fresh reading with windows and a positive raw credit balance, or an explicitly unlimited balance, appears on the provider card as `balance · ...` beside those windows, including a card whose window relationship is unmeasured; in practice Codex, Grok and Command Code report this state. It is display-only: zero, stale, and failed readings omit the line, and no percentage or bound is derived from it. A per-key spend-cap window remains the source of its own row rather than being duplicated as a balance. Credits-only readings retain their balance-only card behavior.
...
When stale or unavailable quota is likely fixable by a one-time native secure-store grant, `state.reason` is `keychain_access_required`, `state.remedyCommand` supplies the runnable grant command (Copilot's includes `--provider copilot`), and for Claude and Cursor JSON also includes an agent-directed `help` entry. That remedy is offered only for `keychain_prompt_required`; backend-specific consent behavior is described in [Provider notes](#provider-notes). A denied or timed-out Keychain outcome is reported as `keychain_access_denied` or `keychain_prompt_timeout` and stays silent no longer, but it does not advertise `--allow-keychain-prompt`. For Claude, macOS `security` exit 44 is cannot-reach (`keychain_unreachable`), not item-absent: it is not signed-out, and it does not retire the Claude cache. A successful metadata listing covering every keychain in the search list, with no matching or unrecognized Claude item, is the only evidence that establishes a missing Claude Keychain source; an empty, partial, or failed listing proves nothing and falls back to the exact profile-and-account read.
...
| ---------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Claude                 | Can report `five_hour`, `seven_day`, optional `seven_day_opus`, and optional `extra_usage` windows. Claude's OAuth usage `utilization` values, and scoped-limit `percent` values, are remaining percentages; quota-axi stores their complement as `percentUsed` (while `extra_usage.utilization` remains its vendor-defined used-credit percentage). Trusted session/weekly/model windows emit fixed `windowSeconds` (18,000 or 604,800) for pace; `extra_usage` does not invent a monthly duration.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
| Grok                   | With a consumer-compatible Grok session or Pi `xai` OAuth bearer, can report the shared `credits`-identified current-period window (kind `weekly`/`monthly` from the decoded period type, otherwise `credits`; labeled `week`/`month` accordingly), optional product-scoped `product:<slug>` windows with the same kind, the current-period `startsAt` and reset, and optional prepaid credit balance from the consumer Usage-page operation. Official Grok Build OAuth and Pi OAuth can remain valid for their model catalogs while that separate operation rejects their audience; in that case quota-axi reports usable auth with no windows rather than inventing a SuperGrok percentage. A Pi `xai` API key likewise establishes model usability only. Top-level `credits.remaining` is prepaid/on-demand balance only; it is not subscription remaining and does not bound the period windows used for effective availability. Pace prefers the startsAt/resetsAt pair.                                 |
...
| OpenCode Go            | Using the credential sources documented in [Security Posture](#provider-credential-sources), reports the provider's rolling, weekly, and monthly usage windows as stacked plan caps that jointly bound Go-plan usage at `all_models` scope. It applies the plan's declared cycle lengths ($12 per rolling 5 hours, $30 per week, $60 per calendar month) when the payload carries no duration of its own: rolling and weekly windows gain trusted 18,000s and 604,800s durations, and the monthly window derives a start one calendar month before its reported reset (day-of-month clamped); a payload-supplied duration still wins, so pace, runway, and selection are measurable. A zeroed plan window blocks Go-plan requests, but the vendor's free-model fallback or an opted-in Zen balance may still serve past it.                                                                                                                                                                                   |
...
| GitHub Copilot | `$GITHUB_COPILOT_APPS_JSON` when set or the local Copilot apps auth file; on macOS and Windows, the Copilot CLI's secure-store item for the default `~/.copilot/config.json` selection (Keychain service `copilot-cli`, or Credential Manager target `<host>:<login>.copilot-cli`) with `--allow-keychain-prompt` or a selection-scoped marker; then only the `github.com` host `oauth_token` from the GitHub CLI's `hosts.yml` (`$GH_CONFIG_DIR`, else `$XDG_CONFIG_HOME/gh`, else `%AppData%\GitHub CLI` on Windows, else `~/.config/gh`) |
...
- When that stored token was expired, carried a refresh token, and was definitively rejected, quota-axi may run `claude doctor` and retry once against the store Claude Code rewrote, subject to the best-effort live-process check and residual race described under [Delegated credential refresh](#delegated-credential-refresh). `claude doctor` is Claude Code's own installation health check: it starts no session, sends no model request, spends no quota, and does not connect to configured MCP servers. quota-axi never exchanges the Claude refresh token itself; it only checks that one is present, because Anthropic rotates it on use and a second exchange would sign Claude Code out. On macOS a withheld Keychain value read suppresses the delegate and keeps the Keychain remedy instead, because the CLI would rewrite a store quota-axi still could not read. An HTTP 401 on a stored-expired token that carries refresh-token presence is soft expiry, not a sign-out: it reports `state.authStatus: expired_refreshable` with `state.status: unavailable` or stale-cache fallback (`Claude access token expired`) and never retires the Claude cache. That covers delegation being suppressed (Claude Code already running, refresh disabled with `--no-credential-refresh`, `claude` not found, an unlistable process table, or a withheld Keychain read) and a delegated run that outran its wait. That includes a delegated `claude doctor` that genuinely ran and left the same refreshable token rejected: that read stays soft too, and additionally carries `state.reason: credentials_expired`, `state.remedyCommand: claude` and an agent-directed JSON `help` entry telling the user to run Claude Code once (the Claude counterpart of Grok's remedy). When the Claude CLI was not found or could not be started, the same remedy names that limitation and the help instead says quota-axi could not run the Claude CLI. The remedy is not emitted while Claude Code is running or the delegate outran its wait. Among resolved (non-transient) rejections the highest-priority source's verdict wins, so a leftover non-refreshable `.credentials.json` cannot override a refreshable Keychain session listed first. `auth_required` therefore requires a 401 on a non-refreshable or stored-valid candidate that leads the source order, or absence proven from complete metadata. A denied or unreachable Keychain read still surfaces as the Keychain error rather than a sidecar's soft expiry. The confirmed-expiry 429 path above uses the same `expired_refreshable` status when its token carries a refresh token, under its own `Claude credential expired` message.
- Missing or invalid credentials without a usable access token and usage HTTP 401 (except the refreshable stored-expired case above) normally bypass and best-effort retire Claude cache. For stored credentials, when macOS Keychain discovery fails or its value read is denied/unreachable, a 401 from an oauth-file sidecar rejects only that candidate: quota-axi reports the Keychain failure, does not classify Claude as signed out, and preserves any eligible stale cache. Only complete search-list metadata with no matching or unfamiliar Claude item reports a missing source; every exact-read failure, including exit 44, remains a Keychain failure that preserves the cache. Stored-credential HTTP 403, timeout, network, rate-limit, server, and response-compatibility failures may use only a formerly fresh Claude snapshot less than seven days old that was captured for the same locally selected Claude configuration context (see [Cache](#cache)). Reset-expired windows are removed; resetless session, monthly, and credit windows expire after five hours, resetless weekly and model windows expire after seven days, and resetless unknown windows are rejected. Env-selected failures never serve stale quota. A stale Claude attention row names `fetch failed` plus the usage-fetch error rather than a bare stale label.
...
- It checks three credential stores in order. Copilot's own `apps.json` is first, and a working `apps.json` reads exactly as before. The standalone Copilot CLI sign-in in the platform secure store (`copilot-cli:keychain`) is second. The GitHub CLI (`gh`) login in `hosts.yml` is third, because current Copilot sign-ins no longer write `apps.json` and the Copilot CLI accepts a `gh` OAuth token as a Copilot credential. A reading from the Copilot CLI sign-in uses top-level source `cli`; a reading from the GitHub CLI login names `gh:hosts.yml` in `sourcesTried`.
- quota-axi hands over to the next store only when the previous one is absent, cannot be used, or its token gets HTTP 401 or 403 that is not a rate limit. A server, network, decoding, or rate-limit failure stops the search. When the GitHub CLI login answers, a present `apps.json` that did not answer shows as a degraded source. A rejected `apps.json` token keeps its attempt name `api`, as before. The stores are independent: `apps.json` or the `gh` login can belong to a different account from the Copilot CLI's selected one.
- The Copilot CLI source is supported on macOS and Windows for the default profile only (`~/.copilot/config.json`, or `%USERPROFILE%\.copilot\config.json`), and only for an exact `lastLoggedInUser` whose host is `https://github.com`. On macOS it reads the Keychain item with service `copilot-cli` and account `<host>:<login>`. On Windows it reads one generic credential (type 1) through `CredReadW`, with target `<host>:<login>.copilot-cli` and username `<host>:<login>`; the `LegacyGeneric:target=` prefix that `cmdkey` shows is display notation, not part of the target. The source name `copilot-cli:keychain` covers both backends. After the value read, the selection is read again, and a changed selection discards the token. quota-axi never picks another logged-in user, never reads the CLI's plaintext `copilotTokens` fallback, and never launches `copilot`, refreshes, or migrates storage.
- These bindings were checked empirically with Copilot CLI **1.0.87-0** on macOS and **1.0.86** on Windows x64. GitHub's [authentication documentation](https://github.com/github/docs/blob/733a8a36df30c11712cdf691a5a9296b669172d5/content/copilot/how-tos/copilot-cli/set-up-copilot-cli/authenticate-copilot-cli.md) documents the service and multiple-account behavior but not the item schema, so they are not a stable vendor contract. Selections quota-axi cannot bind with certainty are reported rather than guessed: a non-default `COPILOT_HOME` (`copilot_home_unsupported`; metadata is inspected there, and no default profile is substituted), another host spelling or an enterprise host (`selected_host_unsupported`), an absent or malformed selection (`selected_account_unconfirmed`), and Linux or another platform without a supported store (`secure_store_unsupported`). A non-blank `COPILOT_GITHUB_TOKEN`, `GH_TOKEN`, `GITHUB_TOKEN`, `COPILOT_GH_HOST`, or `GH_HOST` reports `environment_selection_unsupported`, because the CLI would select through the environment instead; only presence is checked, and a blank value selects nothing.
- Consent follows the Claude and Cursor CLI Keychain sources. Plain macOS calls check item metadata only until `--allow-keychain-prompt` succeeds; approve the Keychain request with "Always Allow". On Windows, `CredReadW` returns the secret together with its metadata, so plain calls inspect only the CLI config, and the same flag permits the read without promising an OS prompt. A successful read records a non-secret marker, hashed over the config path, service, and account (`0600` on POSIX), so later quota calls for that same selection read without the flag. Ordinary `auth` stays metadata-only even with a marker. The prompt remedy (`state.remedyCommand: quota-axi --provider copilot --allow-keychain-prompt`) is offered only for `keychain_prompt_required`; macOS exit 44 is `keychain_item_unavailable`, not a sign-out.
- The Windows read runs the fixed system Windows PowerShell path with no profile, non-interactively, with a 10-second timeout and bounded output. It checks the returned target, username, and type, and accepts only a bounded literal GitHub token. `credential_not_found`, `credential_logon_session_unavailable`, `credential_access_denied`, `credential_binding_mismatch`, `credential_format_unsupported`, `credential_read_timeout`, and `credential_read_failed` stay distinct unmeasured outcomes and never imply sign-out. A service or SSH session without the desktop user's logon session cannot establish that the credential is absent. Windows ARM64 has not been checked.
- A rejected Copilot CLI token reports `GitHub Copilot credential rejected or quota access denied` on its attempt and hands over to the `gh` login. Copilot CLI readings are never written to the cache or reused as stale. When that source could have answered (its config selects an account on a supported platform, including an item still awaiting `--allow-keychain-prompt`, or the config cannot be read with certainty), an older `apps.json` or `gh` snapshot is withheld too, so another account's quota is never served while the selected account is unmeasured. An item awaiting the opt-in reports `unavailable` with `keychain_prompt_required` and the prompt remedy, never a sign-out, and does not use an older stale snapshot. A missing config, no selection, or an unsupported platform keeps the existing stale fallback, and also keeps the Copilot CLI source from defining the verdict.
...
- It only sends tokens associated with public GitHub hosts to that public endpoint; host-specific GitHub Enterprise tokens are treated as unavailable there. Redirects are rejected.
- No Copilot store is refreshed, and Copilot has no delegated refresh.
...
- It sends one redirect-disabled `GET` to that host's `/api/monitor/usage/quota/limit` with the key in a Bearer `Authorization` header, a 15 second total deadline, and a 262,144-byte decoded-body cap. Direct requests take the host's default dual-stack path first and retry once over IPv4 only when that attempt fails to connect, because the vendor advertises IPv6 addresses that an IPv4-only host cannot reach. A configured HTTP(S) proxy still takes precedence. The endpoint is undocumented, so normalization is deliberately schema-tolerant rather than positional. A definitive rejection for one store, either HTTP 401/403 or the vendor's HTTP 200 `{"code":1000,"success":false}` authentication-failed body, falls through to the next store; transport, timeout, rate-limit, and server failures stop without credential switching.
...
- It sends one redirect-disabled `GET` per attempted credential to DeepSeek's first-party `/user/balance` endpoint with a bearer token, trying the environment key before Pi under the [credential handover rules](#provider-state). The balance representation is documented under [Provider windows](#provider-windows).
...
| Reports and details not cached         | Failed providers, stale providers, account identity, and source attempts are not cached. Claude native inference readings and Copilot native secure-store readings (`source: cli`) are excluded from both cache writes and clears, leaving existing stored-session snapshots untouched.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| Kimi cache provenance                  | Follows the [Kimi provider rules](#provider-notes): a snapshot is reused only when its opaque SHA-256 identifier still matches the source that produced it - the selected Kimi Code environment, or the Pi source and its default endpoint - and legacy context-less Kimi records are not reused. A reusable snapshot written before `shareOf` existed stores `month_code` with `percentUsed` alone, so reading it restores the `shareOf: month_total` marker; a stale fallback after upgrading therefore still presents that window as a used share rather than as missing data.                                                                                                                                                                                                                                                                                                           |
| Command Code cache provenance          | Follows the [Command Code provider rules](#provider-notes): a snapshot is reused only after the current `whoami` identifies the account and the opaque SHA-256 source-plus-account identifier still matches; a failed or unidentified `whoami` cannot select stale data from an unknown account and does not cache or clear the existing record.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| MiniMax cache provenance               | Follows the [MiniMax provider rules](#provider-notes): a snapshot is reused only when its opaque SHA-256 identifier still matches the credential source and deployment host that produced it, and legacy context-less MiniMax records are not reused.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ElevenLabs cache provenance            | Follows the [ElevenLabs provider rules](#provider-notes): each snapshot carries an opaque SHA-256 of the answering source plus a one-way digest of the key that produced it, and stale fallback requires that identity to match. A reading with no usable key neither caches nor clears the record, and a cached window whose own reported reset has already passed is never served.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
