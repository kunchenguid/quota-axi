...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), and Kiro quota windows in one [AXI](https://axi.md)-shaped call.
...
auth[18]{provider,source,path,status,error}:
  ...
  kiro,kiro-sqlite,~/.local/share/kiro-cli/data.sqlite3,available,none
help[1]:
...
| Flag                                                                                | Description                                                               |
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,kiro` | Scope providers                                                           |
...
Alibaba's account `weekly` window is reported at `all_models` scope, while each `model:*` limit is kept only at its named model scope; a model limit never becomes an account-wide bound. OpenCode Go's rolling, weekly, and monthly windows are stacked plan caps ($12 per rolling 5 hours, $30 per week, $60 per month) that jointly bound Go-plan usage at `all_models` scope, so effective remaining is the minimum across them. A zeroed plan window blocks Go-plan requests, but the vendor's free-model fallback or an opted-in Zen balance may still serve past it, which the usage endpoint does not report. Kiro's normalized `credit` window alone bounds usage at `all_models` scope. It combines base usage and allowance with `freeTrialInfo` when its status is `ACTIVE` and its reported expiry has not passed; an absent or unparseable expiry does not exclude an active trial. Each pool's usage is capped at its own allowance before aggregation. An incomplete active trial withholds the breakdown instead of reporting base credits alone. The normalizer accepts the vendor's canonical camelCase response fields, prefers `currentUsageWithPrecision` and `usageLimitWithPrecision`, and preserves percentage precision through effective availability so a positive balance cannot become `exhausted_now` through display rounding. Any unfamiliar breakdown leaves effective availability unknown rather than being folded into the credit bound. The endpoint reports a reset but no cycle start or duration, so cycle-based pace, runway projections, and selection remain unknown.
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `gh:hosts.yml`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, `kiro-sqlite`, and `loopback` |
...
| Kiro           | The current user's Kiro CLI SQLite auth store at `$KIRO_CLI_DB` or `~/.local/share/kiro-cli/data.sqlite3`; quota-axi reads only the `kirocli:odic:token` record read-only. `KIRO_REGION` overrides the token's region, defaulting to `us-east-1` when neither is set. `KIRO_PROFILE_ARN` optionally selects an IAM/profile account.                                                                                                                                          |
...
- It opens the database selected under [Provider credential sources](#provider-credential-sources) read-only. An absent store is `missing` and is never opened; `node:sqlite` is loaded only after the file exists, avoiding its experimental warning on Node versions that emit one when Kiro is uninstalled. A file that is not a database, a store without the `auth_kv` table, a malformed token record, or one without an `access_token` is `invalid` (`credentials_invalid`, a present credential that does not work); a locked or busy store is a transient `error` that preserves the cache on disk rather than a sign-out.
- It sends the short-lived access token only as a bearer to Kiro's first-party `AmazonCodeWhispererService.GetUsageLimits` endpoint, with a fixed read-only request body (only the optional profile ARN; account email is not requested) and no cookies. The request rejects redirects, uses a 15 second timeout, and caps the response body at 262,144 bytes before JSON parsing. Oversized or malformed JSON responses report a provider error.
- Stored `expires_at` is advisory: a stored-expired token is still probed, and an empirically live one reports fresh quota. When the endpoint rejects it (401/403) and the record carries a `refresh_token` (presence only, never read), the result is `state.authStatus: expired_refreshable` with `state.status: unavailable`, keeping the cache, because Kiro CLI refreshes lazily on its next use. A rejected stored-valid token, or a rejected stored-expired one without a refresh path, is `auth_required` with `Kiro sign-in required`.
- Kiro snapshots carry two opaque SHA-256 configuration identifiers: `credentialContext` covers the resolved database path, region, and optional profile identity; `credentialRetirementContext` covers the path and optional profile identity without region. They contain no raw path or profile ARN, credential or token hash, or account identifier. Definitive credential failures (a missing token record after `kiro-cli logout`, an invalid store, or a first-party 401/403 that is not soft expiry) retire only a matching retirement context and, when the region is known, a matching reading context. This permits retirement after logout with `KIRO_REGION` unset and the token row gone. Retirement is best-effort; a deletion failure still withholds cached quota. Failed reads never reuse cached plan or credits: the current credential reader supplies no verified non-secret account-continuity evidence, and an unchanged database, region, and profile can hold a different account between reads. Transient failures preserve the cache on disk and report the current error, including retry guidance when available.
- It never refreshes, writes, or deletes Kiro credentials; it never launches `kiro-cli`. Token, email, profile ARN, and refresh-token values are excluded from reports, cache, and errors; tests use synthetic credentials.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity, Kiro | none                              | read-only; see the per-provider notes below           |
...
Providers with no established non-interactive rotation command stay read-only on purpose. That is a documented limitation rather than a reason to force an unsafe path: Cursor's CLI token is long-lived and no non-interactive `cursor-agent` command was observed to rotate it, GitHub Copilot's stored OAuth tokens (in `apps.json` and the GitHub CLI's `hosts.yml`) do not expire, Z.AI uses a non-expiring API key, Alibaba is accessed through the read-only `bl` usage command, OpenCode Go has no vendor-owned rotation command, Kiro CLI refreshes its token lazily on its own next use and no non-interactive `kiro-cli` rotation command was observed, Pi-owned OAuth entries (`openai-codex`, `xai`, `kimi-coding`) have no non-interactive Pi refresh command, and Antigravity exposes no credential store at all.
...
| Fresh provider reports with no windows | Clear the cached snapshot for that provider so entitlement-only reports do not leave stale quota windows behind. For context-scoped providers (Claude, Kimi, and Kiro), clearing requires a defined matching reading context; snapshots from other or legacy contexts are preserved.                                                                                                                                                                                                                                                                                         |
