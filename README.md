...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity (`agy`), and Command Code quota windows in one [AXI](https://axi.md)-shaped call. MiniMax and OpenRouter are also supported as opt-in providers selectable with `--provider minimax` or `--provider openrouter`; they never appear in a default probe.
...
| MiniMax (opt-in)       | Reports the supplied per-model `model:<name>:interval` and `model:<name>:weekly` Coding Plan meters from valid remaining percentages or count totals. It preserves supplied reset timestamps and remaining-duration text, but malformed, out-of-range, or incoherent meters are named in `state.untrustedWindowIds` rather than published. The vendor does not establish whether per-model meters are independent or jointly bound, so effective availability stays unknown.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| OpenRouter (opt-in)    | Reports one `credits` window with `spentUsd` and `limitUsd` only when the key response supplies numeric `usage` and a positive `limit`; a missing, null, or zero limit, or missing usage, remains auth-usable with no invented window. The endpoint supplies no reset or bound relationship, so pace, runway, and effective availability stay unknown.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |

### Model catalog and `models`

`quota-axi models [--intelligence high|medium|low] [--sort runway] [--provider ...] [--json|--full]` joins a reviewed catalog of native Claude, Codex, Grok, and Kimi models to the provider's effective quota evidence. It queries those four catalog-backed providers by default and accepts only those providers in an explicit models scope. Cursor and Copilot are excluded from this first catalog because their hosted model availability is plan-dependent; Copilot's quota relationships are also currently unknown. Z.AI, Alibaba, OpenCode Go, Antigravity, Command Code, MiniMax, and OpenRouter report quota but have no reviewed catalog entries yet, so they are not `models` providers either.
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `gh:hosts.yml`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:openai-codex-*` sibling keys, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `pi:commandcode`, `pi:minimax`, `pi:openrouter`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, `loopback`, `env:COMMAND_CODE_API_KEY`, `env:COMMANDCODE_API_KEY`, `commandcode-cli`, `omp:commandcode`, `MINIMAX_API_KEY`, and `OPENROUTER_API_KEY` |
...
| Z.AI                | Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) for a literal Coding Plan `api_key` entry under `zai`, then opencode's `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `$LOCALAPPDATA/opencode/auth.json` on Windows when available, then `~/.local/share/opencode/auth.json`) for a literal key under `zai-coding-plan`, `zai`, `z-ai`, `z.ai`, `zhipu`, or `zhipuai`                                                          |
...
| MiniMax (opt-in)    | `$MINIMAX_API_KEY` first, then opencode `auth.json` (`$XDG_DATA_HOME/opencode/auth.json` when set, otherwise `$LOCALAPPDATA/opencode/auth.json` on Windows when available, then `~/.local/share/opencode/auth.json`) for a literal key under `minimax`, `MiniMax`, or `minimax-coding-plan`; Pi's `$PI_CODING_AGENT_DIR/auth.json` entry under the same ids (literal `api_key.key` or `oauth.access`) is consulted after opencode. Opt-in: not in the default probe list.    |
| OpenRouter (opt-in) | `$OPENROUTER_API_KEY` first, then opencode `auth.json` for a literal key under `openrouter`; Pi's `$PI_CODING_AGENT_DIR/auth.json` `openrouter` entry (literal `api_key.key` or `oauth.access`) is consulted after opencode. Opt-in: not in the default probe list.                                                                                                                                                                                                          |
...
- For each credential probed, MiniMax's opt-in Coding Plan adapter sends one no-cost, redirect-disabled `GET` to `https://api.minimax.io/v1/api/openplatform/coding_plan/remains` with a 15 second per-request deadline and a 262,144-byte decoded-body cap. It reports the vendor's per-model time- or count-metered allowance without calling inference or deriving quota from model lists.
- It checks three independent sources in a fixed order: `$MINIMAX_API_KEY` first (explicit caller intent), then opencode `auth.json`, then Pi's `$PI_CODING_AGENT_DIR/auth.json`, for a literal key under `minimax`, `MiniMax`, or `minimax-coding-plan`. A Pi OAuth record is accepted only when its `access` is a usable literal bearer; stored expiry is advisory and the record is still probed.
- HTTP 401/403 and vendor `base_resp.status_code: 1004` reject the credential; other nonzero vendor status codes are request failures, and HTTP 429 is rate-limited. Request, transport, timeout, and decoding failures stop credential handover. Valid vendor meters produce windows only from supplied remaining percentages or coherent counters: out-of-range percentages and over-total or inconsistent counters are named in `state.untrustedWindowIds` rather than clamped. Time-metered reset timestamps accept epoch seconds or milliseconds, and vendor remaining durations are milliseconds.
- A live reading with no supplied meter publishes `authStatus: usable` with `windows: []`. When credential rejection defines the provider verdict, it reports `auth_required`. Absent sources produce `auth_required` only when every source is absent; an unresolved unreadable source takes precedence over credential rejections when no source succeeds, mirroring the cross-provider rule that absence alone never degrades a sibling source.
- The MiniMax adapter neither reads nor deletes cached snapshots and offers no stale-cache fallback. The CLI’s shared cache writer saves fresh readings with windows and clears the prior snapshot on a fresh reading with no windows. Errors preserve existing records: a successful read followed by HTTP 401 leaves the prior snapshot on disk, but the adapter does not return it.
- A structurally invalid present-but-broken entry is named `credentialPresent: true` so a working sibling is never hidden behind it.
- It never logs in, exchanges a refresh token, calls inference, derives a quota percentage from model presence, or sends cookies. MiniMax API keys do not expire on a documented schedule, and no vendor CLI was observed to rotate one, so MiniMax has no delegated refresh.

**OpenRouter (opt-in)**

- OpenRouter publishes a single bounded, read-only `GET /api/v1/auth/key` endpoint that returns the key's `label`, `usage`, and (when set) its `limit`. quota-axi reports exactly that: a `credits` window with `spentUsd` and `limitUsd` when `data.usage` is numeric and `data.limit` is a positive number, and `authStatus: usable` with empty windows when the limit is `null`, missing, or zero, or usage is unavailable. A numeric `usage` without a positive `limit` never becomes a percentage - matching the Grok Pi OAuth "live_no_quota" path.
- It checks three independent sources in a fixed order: `$OPENROUTER_API_KEY` first (explicit caller intent), then opencode `auth.json` for a literal key under `openrouter`, then Pi's `$PI_CODING_AGENT_DIR/auth.json` for a literal `api_key.key` or `oauth.access` under `openrouter`. A Pi OAuth record is accepted only when its `access` is a usable literal bearer; stored expiry is advisory and the record is still probed.
- Each source sends one redirect-disabled `GET` to `https://openrouter.ai/api/v1/auth/key` with a 15 second per-request deadline and a 262,144-byte decoded-body cap. Only HTTP 401 or 403 is a definitive auth rejection for that key; HTTP 429 is rate-limited, every other non-2xx status and every transport/timeout/decoding failure is a non-credential error and stops credential handover.
- The endpoint does not return a reset timestamp, so `pace` and `runway` for the `credits` window stay `unknown`. A read that returns `usage: 0, limit: null` reports the key as `authStatus: usable` with no windows, not as a zero-balance credential; a zero limit is treated as no-quota for the same reason.
- When credential rejection defines the provider verdict, it reports `auth_required`. Absent sources produce `auth_required` only when every source is absent; an unresolved unreadable source takes precedence over credential rejections when no source succeeds, mirroring the cross-provider rule that absence alone never degrades a sibling source.
- The OpenRouter adapter neither reads nor deletes cached snapshots and offers no stale-cache fallback. The CLI’s shared cache writer saves fresh readings with windows and clears the prior snapshot on a fresh reading with no windows. Errors preserve existing records: a successful read followed by HTTP 401 leaves the prior snapshot on disk, but the adapter does not return it.
- A structurally invalid present-but-broken entry is named `credentialPresent: true` so a working sibling is never hidden behind it.
- It never logs in, exchanges a refresh token, calls inference, sends cookies, or exposes account identity beyond the key `label`. OpenRouter API keys do not expire on a documented schedule, and no vendor CLI was observed to rotate one, so OpenRouter has no delegated refresh.
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, Antigravity, Command Code, MiniMax, OpenRouter | none                              | read-only; see the per-provider notes below           |
...
Providers with no established non-interactive rotation command stay read-only on purpose. That is a documented limitation rather than a reason to force an unsafe path: Cursor's CLI token is long-lived and no non-interactive `cursor-agent` command was observed to rotate it, GitHub Copilot's stored OAuth tokens (in `apps.json` and the GitHub CLI's `hosts.yml`) do not expire, Z.AI, MiniMax, and OpenRouter use non-expiring API keys, Alibaba is accessed through the read-only `bl` usage command, OpenCode Go has no vendor-owned rotation command, Pi-owned OAuth entries (`openai-codex`, `xai`, `kimi-coding`, `commandcode`) have no non-interactive Pi refresh command, Command Code API keys do not expire, and Antigravity exposes no credential store at all.
