...
- Credential storage follows Claude Code's selectors: a defined `CLAUDE_SECURESTORAGE_CONFIG_DIR` overrides `CLAUDE_CONFIG_DIR` for both `.credentials.json` and the Keychain service. A nonempty storage override uses that directory; an explicitly empty override uses `~/.claude` and the unsuffixed `Claude Code-credentials` service. Without the storage override, the credential directory remains `$CLAUDE_CONFIG_DIR` or `~/.claude`; only a nonempty `CLAUDE_CONFIG_DIR` adds a suffix. Suffixed services are exactly `Claude Code-credentials-<sha256(NFC storage path).slice(0,8)>`. The hash uses the raw NFC-normalized path, without resolving relative paths, expanding `~`, or trimming it, matching Claude Code's selector.
- quota-axi reads `security list-keychains`, then lists metadata from those paths in one `security dump-keychain <search-list paths...>` call (no values, raw data, or ACLs). Each command is bounded to five seconds and 16 MiB. It accepts only generic-password records for the validated current-user account whose service names the selected credential directory: the exact selected service, or the one equivalent spelling of that same directory (see below). The exact selector is preferred when both exist. Duplicate matching items follow Keychain search-list order, independently of modification time or dump order; each credential pass reads at most one value, pinned to the located keychain, service, and account.
- An arbitrary eight-hex suffix does not identify the default profile: the same naming shape can represent an explicit profile or an MCP OAuth store. Such items are never opened merely because they are newer or are the only suffixed item. The single exception is deterministic rather than heuristic: because the unsuffixed service is what Claude Code writes for `~/.claude`, when `~/.claude` is the selected credential directory the suffixed `Claude Code-credentials-<sha256(NFC ~/.claude).slice(0,8)>` and the unsuffixed service name that same directory, so each is accepted in place of the other. That covers a default read of credentials a session wrote with `CLAUDE_CONFIG_DIR` set to the default path. No other suffix is accepted. No explicit profile falls through to another profile's item. If the configured service cannot be located and other Claude-prefixed items remain, the result stays inconclusive and eligible stale cache is preserved.
- Metadata discovery runs again on each credential pass, including every TUI refresh. A replacement under the selected service, a changed search list, or a changed storage selector is picked up without restarting. A new suffix with no matching configured selector remains unconfirmed; recency cannot establish its ownership. A newly selected service still requires its own access marker or `--allow-keychain-prompt`.
