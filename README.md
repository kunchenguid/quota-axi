...
- The access token value is used only as the bearer of Cursor's read-only dashboard RPCs (`GetCurrentPeriodUsage`, `GetPlanInfo`, and `GetSandUsageStatus`) and optional first-party `/auth/full_stripe_profile` identity read. It is never logged, hashed, cached, or included in output. A stable account/customer identifier returned by these first-party responses is exposed only in `--full` as verified account identity; email alone is unverified. Fresh Cursor cache entries are scoped by an opaque hash of that remote identity (never of the credential), and stale quota is reused only when a current remote response proves the same account. Context-less legacy entries and responses without remote identity evidence are not reusable, so switching between editor, CLI, and Pi can never serve another account's snapshot. quota-axi does not call Grok Bot trial, banked-reset, or machine-registration methods.
...
| Cursor cache provenance                | A fresh snapshot is cached only when a first-party Cursor response supplies stable account identity. The cache stores only an opaque SHA-256 account context (never the raw identity or credential), and stale reuse requires matching current remote identity evidence even when the credential source changed. Legacy and context-less Cursor entries are not reused.                                                                                                                                                                                                      |
