...
Kimi's weekly, five-hour, and monthly-total windows jointly bound model usage at `all_models` scope, while the monthly code window is a separate `code` resource, so those two monthly meters are never folded into one number. An unfamiliar or unparsed Kimi limit stays named in `unresolvedWindowIds` and turns the provider's semantics `partial`.
