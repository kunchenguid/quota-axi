...
**Credential-source note:** Claude Code keeps live tokens in the macOS Keychain. The Cursor CLI (`cursor-agent`) file store keeps its access token in `~/.cursor/auth.json` on macOS (`AGENT_CLI_CREDENTIAL_STORE=file`) and in `~/.config/cursor/auth.json` on Linux (or the XDG/`$CURSOR_CLI_AUTH_FILE`/`$CURSOR_CLI_CONFIG` override); otherwise macOS Cursor CLI tokens live in the Keychain.
quota-axi does not read macOS Keychain values until the user grants permission, so Claude quota can stay stale and Keychain-only Cursor CLI auth can appear unavailable when no other usable credential exists. The Cursor CLI file store is read without that grant. quota-axi reads only the auth file's `accessToken` and never its refresh token.
...
| Cursor         | Cursor editor: `$CURSOR_STATE_DB` when set or the platform Cursor state database path. Cursor CLI (`cursor-agent`) file store: only `accessToken` from `$CURSOR_CLI_AUTH_FILE`, macOS `~/.cursor/auth.json`, or Linux `$CURSOR_CLI_CONFIG` / `${XDG_CONFIG_HOME:-~/.config}/cursor/auth.json`. macOS Keychain fallback: identity from `$CURSOR_CLI_CONFIG` or `~/.cursor/cli-config.json` plus the `cursor-access-token` / `cursor-user` Keychain value with `--allow-keychain-prompt` or an account-scoped marker |
...
- Cursor Desktop is not required. On macOS, a CLI-only machine can refresh from the file store at `~/.cursor/auth.json` (`cli-authfile`, no Keychain prompt) or from the CLI Keychain token after the one-time Keychain grant described below; those quota attempts are named `cli-authfile` or `cli-keychain` in `sourcesTried`. On Linux, `cursor-agent` quota uses the read-only `cli-authfile` source from `auth.json`; its `accessToken` is only a bearer for the existing dashboard RPCs. The editor-credential fetch keeps its historical `api` attempt name. When credential discovery cannot produce a token, an unavailable source known to hold a credential takes precedence over a merely absent store, so a signed-in `cursor-agent` user sees the applicable source state rather than `Cursor sign-in required`.
...
- CLI source: the read-only `cli-authfile` source is tried first on macOS and Linux. It reads only `accessToken` from `$CURSOR_CLI_AUTH_FILE`, macOS `~/.cursor/auth.json` (`AGENT_CLI_CREDENTIAL_STORE=file`), or Linux `$CURSOR_CLI_CONFIG` / `${XDG_CONFIG_HOME:-~/.config}/cursor/auth.json`; missing, unreadable, malformed, or empty files are unavailable. The sibling refresh token is never read. On macOS, when that file is absent, `cli-config.json` holds sign-in identity only and is never a token; its `authInfo` supplies the reported account email, and the access token is read from the login Keychain item `cursor-access-token` / `cursor-user` only under `--allow-keychain-prompt` or an existing account-scoped non-secret access marker. The `auth` command reports the file store and, on macOS, the Keychain source.
- quota-axi never refreshes Cursor credentials, and Cursor has no delegated refresh. Neither the auth-file `refreshToken` nor the macOS `cursor-refresh-token` Keychain item is read, and no non-interactive `cursor-agent` command was observed to rotate the stored session, so an expired or rejected CLI access token falls through to stale/unavailable reporting and requires `cursor-agent login` outside quota-axi. Cursor CLI session tokens are long-lived (about sixty days observed), so this costs little in practice. This is a known limitation, not a silent gap.
