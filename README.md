...
- Because a reading can come from either Kimi Code deployment, Kimi snapshots are cache-scoped like Claude's: each carries an opaque SHA-256 identifier for the locally selected Kimi Code environment, and stale fallback reuses a snapshot only when that identifier still matches, so one deployment's numbers can never be served for the other. Legacy context-less Kimi records are not reused. The identifier is derived from the resolved slot and base URL, never from `config.toml`'s bytes, so no credential in that file contributes to it and an edit that selects the same environment keeps the cache usable. It discriminates configuration rather than accounts, so it does not distinguish two Pi credentials.
