...
- A credential quota-axi cannot reach is never reported as one the user does not have: a `keyring` slot is `kimi_code_cli_credential_storage_unsupported`, endpoints that are not one of Kimi Code's own deployments are `kimi_code_cli_region_unrecognized`, and configuration naming an unusable slot, or a suffixed slot with no deployment left to place it, is `kimi_code_cli_config_invalid`. A slot quota-axi assumed rather than read, with no file at it, is `kimi_code_cli_credential_unconfirmed`. All four are skipped sources rather than sign-outs, and none retires the cache. Only a slot a real OAuth pointer named, whose file is absent, is the definitive `missing`: a negative from a location quota-axi guessed says nothing about the account, so it never becomes a sign-out.
