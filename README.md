...
`windowSeconds` is a positive number when quota-axi knows the exact period
length and `null` when the window has a period but the provider payload does
not make its exact length knowable. An omitted field is reserved for windows
where a period length is not applicable.
...
| Grok                   | Reports the shared `credits` window, optional product-scoped windows, the current-period reset, and optional prepaid credit balance from the consumer Usage-page operation. Published product IDs remain stable across vendor transports: notably `product:grokbuild` and `product:grokimagine`. The operation does not expose a subscription tier, so `plan` remains absent rather than inferred.                                                                                                                                                                     |
...
### Window duration provenance

| Provider       | Evidence decoded by the adapter                                                                             | `windowSeconds` behavior                                                                                  |
| -------------- | ----------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------- |
| Claude         | Fixed five-hour/seven-day fields and scoped `session`/`weekly` groups; extra usage has no period boundaries | `18000` or `604800` for named limits; `null` for extra usage                                              |
| Codex          | `limit_window_seconds` or `windowDurationMins` when supplied                                                | Exact normalized seconds when supplied; `null` when the source omits duration                             |
| Cursor         | Current-period percentages and a billing-cycle end, but no start or duration                                | `null`; a reset boundary alone cannot establish the exact period length                                   |
| GitHub Copilot | Quota snapshot percentages and reset timestamps, but no start or duration                                   | `null`; a reset boundary alone cannot establish the exact period length                                   |
| Grok           | Weekly/monthly current-period type plus start and end timestamps                                            | Exact boundary difference for a recognized, positive period; `null` for unspecified or incomplete periods |
| Kimi           | The principal weekly contract and self-described duration/unit pairs for additional limits                  | `604800` for the principal window, exact normalized seconds for recognized units, otherwise `null`        |
