...
- Definitive credential failures (a missing token record after `kiro-cli logout`, an invalid store, or a first-party 401/403 that is not soft expiry) retire the Kiro cache and never serve a stale credit window; only transient transport, timeout, rate-limit, busy-store, and `expired_refreshable` outcomes fall back to the last fresh snapshot.
