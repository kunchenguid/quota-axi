...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:cursor`, `pi:kimi-coding`, `pi:xai`, `pi:zai`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
- The Cursor editor, Cursor CLI, and Pi keep independent credential stores, and any one can answer. Their fixed quota-source order is editor `state-vscdb`, platform CLI, then Pi `cursor`: this preserves the established non-prompting editor/CLI behavior while letting a Pi login recover an absent, unreadable, or definitively rejected system source. Stored expiry never reorders that list. A network, timeout, rate-limit, decoding, or server failure stops handover because it is a request failure, not evidence that another credential should replace the current one. The `auth` command reports all applicable sources.
...
- quota-axi never refreshes Cursor credentials, and Cursor has no delegated refresh. Neither the Linux auth-file refresh token, the macOS `cursor-refresh-token` Keychain item, nor Pi's OAuth refresh value is used, and no non-interactive `cursor-agent` or Pi command was observed to rotate the stored session safely. An expired Pi token remains `expired_refreshable` after definitive rejection because Pi can still own recovery, but quota-axi neither launches Pi nor performs that recovery. Rejected system credentials require signing in again outside quota-axi. This is a known limitation, not a silent gap.
- The access token value is used only as the bearer of Cursor's read-only dashboard RPCs (`GetCurrentPeriodUsage`, `GetPlanInfo`, and `GetSandUsageStatus`) and optional first-party `/auth/full_stripe_profile` identity read. It is never logged, hashed, cached, or included in output. A stable account/customer identifier returned by these first-party responses is exposed only in `--full` as verified account identity; email alone is unverified. Fresh Cursor cache entries are scoped by an opaque hash of that remote identity (never of the credential). A failed live Cursor read stays failed and never serves those cached windows, even when a concurrent identity response names the same account. Context-less legacy entries and responses without remote identity evidence are not reusable, so switching between editor, CLI, and Pi can never serve another account's snapshot. quota-axi does not call Grok Bot trial, banked-reset, or machine-registration methods.
...
Providers with no established non-interactive rotation command stay read-only on purpose. That is a documented limitation rather than a reason to force an unsafe path: Cursor's CLI token is long-lived and no non-interactive `cursor-agent` or Pi command was observed to rotate Cursor auth safely, GitHub Copilot's stored OAuth token does not expire, Z.AI uses a non-expiring API key, Alibaba is accessed through the read-only `bl` usage command, OpenCode Go has no vendor-owned rotation command, Pi-owned OAuth entries (`openai-codex`, `cursor`, `xai`, `kimi-coding`) have no non-interactive Pi refresh command, and Antigravity exposes no credential store at all.
...
| Cursor cache provenance                | A fresh snapshot is cached only when a first-party Cursor response supplies stable account identity. The cache stores only an opaque SHA-256 account context (never the raw identity or credential). A failed live Cursor read stays failed and never serves that snapshot, even when a concurrent identity response names the account. Legacy and context-less Cursor entries are not reused.                                                                                                                                                                               |
