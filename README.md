...
Kimi's weekly, five-hour, and monthly-total windows jointly bound model usage at `all_models` scope, while the monthly code window is a separate `code` resource, so those two monthly meters are never folded into one number. An unfamiliar legacy limit, and a declared map entry that carries no usable ratio, both stay named in `unresolvedWindowIds` - as `limit:<index>` or `usages:<key>` - and turn the provider's semantics `partial`.
