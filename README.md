...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, Ollama Cloud, and Antigravity (`agy`) quota windows in one [AXI](https://axi.md)-shaped call.
...
attention[4]{provider,scope,kind,detail,remedy}:
  ...
  ollama,all,unresolved_windows,session + weekly,none
...
auth[17]{provider,source,path,status,error}:
  ...
  ollama,"pi:ollama-cloud",~/.pi/agent/auth.json,available,none
  ollama,auth-env,none,available,none
...
| Flag                                                                                  | Description                                                        |
| ------------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `--provider claude,codex,cursor,copilot,grok,kimi,zai,agy,alibaba,opencode-go,ollama` | Scope providers                                                    |
...
- A provider whose window relationships are wholly unknown (Copilot, OpenCode Go, Ollama Cloud, or Antigravity, with every window unresolved) has no combined effective percentage, pace, or runway to show, so its card replaces the headline block with a single `per-window usage · no combined bound` line and leads straight into its real per-window rows. Partially understood providers keep the effective-unknown headline. No combined headroom, pace, or runway number is invented.
...
Alibaba's account `weekly` window is reported at `all_models` scope, while each `model:*` limit is kept only at its named model scope; a model limit never becomes an account-wide bound. OpenCode Go's rolling, weekly, and monthly windows are reported as raw windows but remain `unknown` for effective availability because quota-axi has no provider evidence that those windows jointly bind `all_models`. Ollama Cloud's session and weekly windows are likewise reported as raw windows with unknown joint semantics.
...
| Ollama Cloud           | Reads a literal `ollama-cloud` API key from Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`), then falls back to `OLLAMA_API_KEY`; reports raw session and weekly usage windows without a combined bound or invented reset data.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:ollama-cloud`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
| Ollama Cloud   | `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) for the literal `ollama-cloud` API-key entry, then `OLLAMA_API_KEY` as an environment fallback                                                                                                                                                                                                                                                              |
...
- It never imports browser cookies, scrapes `ollama.com/settings`, launches Ollama, refreshes credentials, writes Pi auth, or caches keys or raw responses. A definitive 401/403 retires the Ollama cache; transient failures preserve an eligible stale snapshot.
