...
| Flag                                                                                            | Description                                                               |
| ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- |
| `--provider claude,codex,cursor,copilot,grok,kimi,meta,zai,agy,alibaba,opencode-go,commandcode` | Scope providers                                                           |
...
- Each provider read has a finite outer bound, including credential discovery and local commands. Live reads use a five-second bound per provider, so an unresponsive source becomes an unavailable card while the other providers still render. Quitting during a pending read restores the terminal immediately.
...
- Width comes from the terminal; cards fill available width and below 100 columns the grid reflows to one column. Below 49 columns, the report uses compact provider and window rows that fit the available width. Color honors `NO_COLOR`, `TERM=dumb`, and non-TTY stdout (the glyph skeleton is kept), re-enables with `FORCE_COLOR`, and uses truecolor when `COLORTERM` advertises it, falling back to 256-color then ANSI-16.
...
Meta Muse Spark's weekly and reported-duration rolling subscription windows are preserved as raw windows. They remain `unknown` for effective availability because Meta has not established that they jointly bind `all_models`; quota-axi does not silently turn either one into a combined account bound.
...
| Meta Muse      | Pi's `$PI_CODING_AGENT_DIR/auth.json` `meta` OAuth entry for a readings-only usage snapshot; the returned Model API key is discarded immediately                                                                                                                                                                                                                                                                                                                             |
...
- It reads only Pi's `meta` OAuth entry from `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`). The misleadingly named `refresh` field is Meta's identity bearer for this operation; the `access` field is an already-minted Model API key and is never used. Stored expiry describes that Model API key and is advisory, so a valid identity bearer is still tested. quota-axi never falls through to the standalone Muse credential store because it cannot prove that store selects the same account.
- It sends one redirect-disabled `POST` with body `{}` to `https://api.meta.ai/muse-code/key`, using the Pi identity bearer and Meta API version `1.0.0`. This is the captain-authorized readings-only exception to the normal no-mint boundary: quota-axi extracts only `subs_usage`, immediately discards the returned Model API key and all other response fields, and never prints, logs, caches, or writes that key, the identity bearer, the raw response, account names, or email addresses. It performs no refresh-token exchange and writes no credential store.
- Valid `subs_usage.weekly` and `subs_usage.window` readings become `weekly` and exact-duration `window:<minutes>m` windows. Percentages must be finite and between 0 and 100, rolling duration must be a positive integer, and invalid resets are omitted rather than invented. Unfamiliar windows use positional `limit:<n>` identifiers so unknown vendor field names cannot leak. A first-party 401/403 is definitive for this single source; transport and decoding failures may reuse only the normalized non-secret cache snapshot.
...
quota-axi reports quota; it is not an auth app. It never rotates a credential or performs a refresh-token exchange over HTTP. Meta's readings-only key call is the sole mint exception and its returned key is immediately discarded. Refresh tokens rotate on use, so a second holder performing the exchange would spend the vendor's own single-use token and sign the user out of the harness being measured.
...
| Cursor, GitHub Copilot, Kimi, Meta Muse, Z.AI, Alibaba, OpenCode Go, Antigravity, Command Code | none                              | read-only; see the per-provider notes below           |
...
- It never rotates or writes a credential, and never performs a refresh-token exchange. Meta's readings-only key call is the sole mint exception; its returned key is immediately discarded. Credential renewal is always delegated to the vendor CLI that owns the store (see [Delegated credential refresh](#delegated-credential-refresh)).
