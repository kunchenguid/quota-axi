...
quota-axi reports local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Z.AI, Alibaba, OpenCode Go, MiniMax, and Antigravity (`agy`) quota windows in one [AXI](https://axi.md)-shaped call.
...
Alibaba's account `weekly` window is reported at `all_models` scope, while each `model:*` limit is kept only at its named model scope; a model limit never becomes an account-wide bound. OpenCode Go's rolling, weekly, and monthly windows are reported as raw windows but remain `unknown` for effective availability because quota-axi has no provider evidence that those windows jointly bind `all_models`. MiniMax's shared `interval` and `weekly` Token Plan windows are reported at `all_models` scope, while each other resource's `model:<name>:interval` / `model:<name>:weekly` pair is kept only at its own `model:<name>` scope; a resource-scoped window never becomes an account-wide bound.
...
| MiniMax                | Reads Pi's `minimax-cn` Token Plan API key and reports the account's shared `interval` and `weekly` windows plus any other resource's own `model:<name>:interval` / `model:<name>:weekly` windows (for example `video`), with durations computed from the endpoint's own timestamps rather than asserting the vendor-documented but unconfirmed "5-hour" duration. A window whose `*_total_count` is zero is omitted rather than reported: the vendor's own CLI maintainers document that field as still reading 100% remaining in that state (no active Token Plan seat, or a resource outside the account's tier), which would otherwise be a false "fully available" reading. The live request can also be rejected by MiniMax's own edge WAF for reasons outside quota-axi's control; see [Provider notes](#provider-notes).                                                                                                                             |

### Model catalog and `models`

`quota-axi models [--intelligence high|medium|low] [--sort runway] [--provider ...] [--json|--full]` joins a reviewed catalog of native Claude, Codex, Grok, and Kimi models to the provider's effective quota evidence. It queries those four catalog-backed providers by default and accepts only those providers in an explicit models scope. Cursor and Copilot are excluded from this first catalog because their hosted model availability is plan-dependent; Copilot's quota relationships are also currently unknown. Z.AI, Alibaba, OpenCode Go, MiniMax, and Antigravity report quota but have no reviewed catalog entries yet, so they are not `models` providers either.
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:minimax-cn`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
| MiniMax        | Pi's `$PI_CODING_AGENT_DIR/auth.json` (default `~/.pi/agent/auth.json`) `minimax-cn` entry, either a literal API key or an unexpired OAuth access token                                                                                                                                                                                                                                                                        |
...
- Stored expiry is advisory, never a verdict. Shared credential selection (`src/providers/credential-selection.ts`) tries stored-valid credentials first, then empirically tests stored-expired ones. A consumer-compatible bearer wins with fresh quota. An official Build/Pi OAuth bearer rejected only by consumer billing is tested against its own first-party model catalog; catalog success produces usable, unmeasurable auth instead of false expiry or sign-out.
- HTTP 401/403 and auth-class gRPC codes are definitive only for the credential surface receiving them. A transient network/rate-limit failure never switches candidates within one credential source and never becomes an auth verdict (the stored classification stands), but it does not prevent trying the other independent source. Such failures remain stale-cache eligible for same-source web snapshots.
...
- MiniMax's own edge WAF (Alibaba Cloud ESA) has been observed returning an HTTP 404 with anti-bot challenge cookies (`acw_tc`, `cdn_sec_tc`) specifically to Node's HTTP client (`fetch`/undici) on this endpoint, while an identical request sent from curl succeeds. This is vendor-side TLS-fingerprint-based blocking outside quota-axi's control, not a header, URL, or credential defect; replaying the challenge cookie does not clear it, and the block has been reproduced consistently across both `www.minimaxi.com` and `api.minimaxi.com`, proxied and direct connections, HTTP/1.1 and HTTP/2, and multiple `User-Agent` values.
- quota-axi reports this as an ordinary transport failure (an honest error, such as `provider_request_rejected` or `network_unavailable`) rather than fabricating data or working around the block: it does not shell out to curl, does not add a TLS-fingerprint-impersonation dependency, and does not otherwise try to evade the WAF. This means MiniMax quota reads can fail for some or all real users until MiniMax addresses it vendor-side; that is an accepted, deliberate limitation, not an oversight.
- This particular failure is a plain HTTP 404, reported as a bare error rather than a stale-cache reuse. In fact, no transient failure ever reuses a cached snapshot for MiniMax: unlike other providers, MiniMax's `failureReport` always reports an honest `error`/`rate_limited` state with no windows for a transport, timeout, rate-limit, or other transient failure, so a WAF block (or any other transient failure) never masks itself behind an old-looking snapshot. Only a definitive `auth_required` rejection still retires the cache; see [Cache](#cache).
...
| Cursor, GitHub Copilot, Kimi, Z.AI, Alibaba, OpenCode Go, MiniMax, Antigravity | none                              | read-only; see the per-provider notes below           |
...
| Claude cache fallback                  | Definitive missing/invalid credential and HTTP 401 failures retire the snapshot. HTTP 403 and other transient failures may use a formerly fresh snapshot from the same selected Claude configuration context, with a seven-day provider bound plus reset and resetless-window pruning. Its cache-only SHA-256 context identifier is opaque; legacy context-less Claude records are not reused. |
...
| MiniMax cache fallback                 | Unlike Claude/Codex/Grok/Z.AI/Kimi above, MiniMax never reuses a cached snapshot to stand in for a failed live read: a transport, timeout, rate-limit, or other transient provider failure reports as an honest `error`/`rate_limited` state instead. A definitive `auth_required` failure still retires the cache, same as other providers.                                                   |
