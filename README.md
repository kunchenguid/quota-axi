...
| `--refresh 30s\|5m\|1h`                                                        | Live `--tui` refresh interval, default 2m (30s-24h)                       |
...
- On an interactive terminal the report stays up and refreshes every 2 minutes until you press `q` (or Ctrl+C), with a `Press q to quit` footer hint. `--refresh` sets the interval (30s-24h) and `--once` renders a single frame. A non-TTY stdout or stdin (pipes, CI, screenshots) always renders one frame and exits.
...
- It never restarts, signs in to, configures, or mutates Antigravity or `agy`. It reads no credential store, so it has no delegated refresh. It prefers running `agy -p "/quota" --output-format json` once with a 15 second bound; this is the vendor's noninteractive read-only usage command (a bounded read, not an agent session or credential operation). When the CLI is unavailable or fails, it falls back to existing loopback access. A CLI that exits without quota still publishes the vendor's own `status`/`error` envelope, so a signed-out or rate-limited session is reported as `auth_required` or `rate_limited` rather than as a malformed read; the envelope's raw text is discarded because a signed-out CLI prints a live OAuth URL. A loopback HTTP 401/403 that names a CSRF token is the loopback's runtime guard, not a sign-out, so it is treated as `unavailable` and retains the cached record. Any reading whose `resetsAt` is in the past is marked stale.
