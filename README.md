...
- Because a reading can come from either Kimi Code deployment, Kimi snapshots are cache-scoped like Claude's: each carries an opaque SHA-256 identifier for the locally selected Kimi Code environment, and stale fallback reuses a snapshot only when that identifier still matches. That separates the two deployments whenever the environment resolves from the configuration, and not when it cannot: an unreadable or unparseable `config.toml` falls back to the same default-slot identity a mainland reading has, so in that one state a mainland snapshot is still served to a global login. Legacy context-less Kimi records are not reused. The identifier is derived from the resolved slot and base URL, never from `config.toml`'s bytes, so no credential in that file contributes to it and an edit that selects the same environment keeps the cache usable. It also comes from the one reading of `config.toml` a run takes before it requests anything, never from a second reading taken afterwards, so a login that switches deployments mid-run cannot file one deployment's numbers under the other's identity; a run that could not read the environment at all stores its snapshot unstamped, which costs only a later stale reuse. It discriminates configuration rather than accounts, so it does not distinguish two Pi credentials.
...
| Kimi cache provenance                  | Follows the [Kimi provider rules](#provider-notes): a snapshot is reused only when its opaque SHA-256 selected-Kimi-Code-environment identifier still matches, and legacy context-less Kimi records are not reused.                                                                                                                                                                   |
