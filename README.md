...
- Access-token JWT usability is authoritative for the native OAuth bearer probe.
  ...
  A stored-expired `access_token` is still probed: the stored `exp` claim orders it behind an unexpired sibling but never skips it, so only the usage endpoint's own rejection is an authentication verdict. A missing `access_token` skips OAuth, and both cases preserve the Pi and read-only CLI fallbacks.
