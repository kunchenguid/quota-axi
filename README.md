...
- For the default profile, quota-axi lists Keychain metadata with `security dump-keychain` (no values, raw data, or ACLs) and considers only generic-password items named `Claude Code-credentials` or `Claude Code-credentials-<8 lowercase hex>`. It selects the most recently modified matching item; unknown modification times sort last, and ties use service then Keychain path in ascending lexical order. The listing is bounded to five seconds and 16 MiB, and each credential pass reads at most one value, pinned to its listed Keychain as well as service and account. Older accumulated items are not retried. Failed or incomplete discovery reports an access/check failure without asserting credential presence.
