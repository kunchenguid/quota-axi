...
- If the Pi source cannot answer, it reads the official Kimi Code CLI credential from the environment `$KIMI_CODE_HOME/config.toml` describes, defaulting to `$HOME/.kimi-code/`. Kimi Code keeps one credential per environment and records which one is current, so quota-axi reads `[providers."managed:kimi-code".oauth]` for that pointer rather than scanning or guessing: it takes only `storage`, `key`, and `oauth_host` from that table and `base_url` from its parent, applies Kimi Code's own key-to-file-name rule plus its containment check, and opens `credentials/<name>.json`. A `config.toml` that is absent, unopenable, past the 256 KiB cap, or naming no OAuth reference is the default slot `credentials/kimi-code.json` - which is what a mainland-China login persists, so that reading is unchanged. Those states share one verdict on purpose: in each, quota-axi does not know what the configuration says, and so is a file carrying a TOML construct this narrow reader cannot walk, which resolves from whatever it read before stopping. An unsupported or unreadable form therefore never withdraws a reading the default slot already supported, and a global login, having no such file, still reports no credential rather than a substituted one. Silence about the deployment is read as the default one for the unsuffixed slot only, since that is the slot Kimi Code writes for it; a suffixed slot with no endpoint left to place it is reported as `kimi_code_cli_config_invalid` rather than assigned a host. It accepts only a non-empty `access_token` with a Unix-seconds `expires_at` (a JSON number or numeric string). A token within 60 seconds of expiry is classified `expired` but is still tested by the bounded read-only probe in the CLI source's declared position; only the endpoint decides the verdict.
- A credential quota-axi cannot reach is never reported as one the user does not have: a `keyring` slot is `kimi_code_cli_credential_storage_unsupported`, endpoints that are not one of Kimi Code's own deployments are `kimi_code_cli_region_unrecognized`, and configuration naming an unusable slot, or a suffixed slot with no deployment left to place it, is `kimi_code_cli_config_invalid`. All three are skipped sources rather than sign-outs, and none retires the cache. A configured slot whose file is absent is still `missing`.
...
- Because a reading can come from either Kimi Code deployment, Kimi snapshots are cache-scoped like Claude's: each carries an opaque SHA-256 identifier for the locally selected Kimi Code environment, and stale fallback reuses a snapshot only when that identifier still matches. Switching deployments rewrites `config.toml` and so changes the identifier, which means one deployment's numbers can never be served for the other; legacy context-less Kimi records are not reused. The identifier discriminates configuration, not accounts, so it does not distinguish two Pi credentials.
