...
| Flag                                                                                | Description                                                        |
| ----------------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| `--provider claude,codex,cursor,copilot,grok,kimi,meta,zai,agy,alibaba,opencode-go` | Scope providers                                                    |
...
| Auth source names    | `oauth-file`, `keychain`, `auth-json`, `auth-env`, `apps-json`, `state-vscdb`, `cli-keychain`, `cli-authfile`, `cli-rpc`, `pi:openai-codex`, `pi:kimi-coding`, `pi:xai`, `pi:meta`, `kimi-code-cli`, `opencode:auth.json`, `bl-cli`, and `loopback` |
...
| Meta Muse      | Pi's `$PI_CODING_AGENT_DIR/auth.json` `meta` OAuth entry (default `~/.pi/agent/auth.json`); only its identity bearer is sent, and only to Meta's first-party key endpoint for the readings-only usage snapshot                                                                                                                                                                                                                 |
...
| Cursor, GitHub Copilot, Kimi, Meta Muse, Z.AI, Alibaba, OpenCode Go, Antigravity | none                              | no delegated renewal; see the per-provider notes below |
...
- It never rotates or writes a credential and never performs a refresh-token exchange. Meta's readings-only key call is the sole mint exception, and quota-axi immediately discards the returned key. Credential renewal is always delegated to the vendor CLI that owns the store (see [Delegated credential refresh](#delegated-credential-refresh)).
- It never retains, prints, logs, renders, caches, or exchanges a refresh token's value. Meta's Pi field named `refresh` is the documented identity bearer for its readings-only request and is the sole send exception. Other Pi credential brokers read a stored refresh value only to derive a usability boolean - whether it is a usable literal secret rather than absent or an environment, template, or command reference - and discard it immediately; elsewhere only its presence is checked, as evidence that the vendor can still recover.
