...
- A rejected refresh token is the definitive end of session and is reported as `Claude sign-in required`, kept distinct from a refreshable expiry. A network, timeout, rate-limit, malformed-response, or concurrent-credential-change refresh failure preserves the valid session and the stored credential unchanged, and stays stale-cache eligible. On a concurrent rotation, quota-axi adopts the newer on-disk credential rather than overwriting it. Access tokens, refresh tokens, authorization headers, and raw token-endpoint responses are never logged, cached, embedded in errors, hashed, or otherwise exposed; `--full` attempts surface only an `oauth-refresh` source with an opaque status code.
