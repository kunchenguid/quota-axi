...
| `--theme light\|dark\|auto`                                                    | `--tui` palette; default `auto` (`QUOTA_AXI_THEME`, then `COLORFGBG`)     |
...
- The truecolor and 256-color palettes come in a dark (Catppuccin Mocha) and a light (Catppuccin Latte) theme. `--theme light|dark|auto` picks one; when the flag is absent `QUOTA_AXI_THEME` (same values) applies, and the default is `auto`. `auto` reads `COLORFGBG` (`fg;bg`) with vim's heuristic: a background index of 7 or 9-15 selects `light`; 0-6, 8 (bright black), 256-color indexes, absent, or unparseable values select `dark`, so a terminal that sets nothing renders exactly as before. A blank `QUOTA_AXI_THEME` is the same as unset. quota-axi never sends an OSC 11 background query, which would add a terminal round-trip to every live refresh. An invalid flag or variable value is a usage error, not a silent fallback. The ANSI-16 path is theme-independent and always uses the terminal's own colors.
