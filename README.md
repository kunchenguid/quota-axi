...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Claude         | `$CLAUDE_CONFIG_DIR/.credentials.json` or `~/.claude/.credentials.json` (not read when a nonempty secure-storage selector is set); on macOS, the exact profile-derived Claude Code Keychain value pinned to Claude Code's validated current-user account, with `--allow-keychain-prompt` or, after a service-and-account-scoped non-secret access marker exists, on plain calls                                                |
...
- With no secure-storage selector, the plaintext credential file is `$CLAUDE_CONFIG_DIR/.credentials.json` or `~/.claude/.credentials.json`. A nonempty `CLAUDE_SECURESTORAGE_CONFIG_DIR` names its own credential store: it selects the Keychain service, and the plaintext file is reported as a skipped `oauth-file` source rather than read, so two profile selectors can never answer for one reading. An empty value falls through to `CLAUDE_CONFIG_DIR`. A nonempty selector gives exactly `Claude Code-credentials-<sha256(NFC selector path).slice(0,8)>`; with neither selector set, the service is the unsuffixed `Claude Code-credentials`. The hash uses the raw NFC-normalized path, without resolving relative paths, expanding `~`, or trimming it, matching Claude Code's selector.
- quota-axi reads `security list-keychains`, then lists metadata from those paths in one `security dump-keychain <search-list paths...>` call (no values, raw data, or ACLs). Each command is bounded to five seconds and 16 MiB. It accepts only generic-password records for the validated current-user account whose service is the exact selected service, or - for a default selection only - the suffixed spelling of `~/.claude` (see below). The exact selected service is preferred when both exist. Duplicate matching items follow Keychain search-list order, independently of modification time or dump order; each credential pass reads at most one value, pinned to the located keychain, service, and account.
- An arbitrary eight-hex suffix does not identify the default profile: the same naming shape can represent an explicit profile or an MCP OAuth store. Such items are never opened merely because they are newer or are the only suffixed item. The single exception is deterministic rather than heuristic: a default selection names `~/.claude`, and so does the suffixed `Claude Code-credentials-<sha256(NFC ~/.claude).slice(0,8)>`, so that one item is accepted when no selector is set. That covers a default read of credentials a session wrote with a selector set to the default path. No other suffix is accepted, and an explicit selector never falls back to the unsuffixed item. No explicit profile falls through to another profile's item. If the configured service cannot be located and other Claude-prefixed items remain, the result stays inconclusive and eligible stale cache is preserved.
