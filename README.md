...
- A nonempty `CLAUDE_CONFIG_DIR` retains the existing exact `Claude Code-credentials-<sha256(NFC config path).slice(0,8)>` lookup. It never falls through to other profiles' discovered items; default-profile discovery does not guess Claude Code's current suffix algorithm.
