...
An account key identifies a **local credential lane**, not a verified person or a guaranteed independent paid allowance. Two profiles can be signed into the same vendor account. `--full` retains the vendor's verified/unverified account evidence and adds `accountLocator` (`kind`, `path`, and optional `entry` / `keychainService` / `delegateEligible`, the last true only for the one lane the process-selected credential owns and therefore the only lane still eligible for delegated refresh) so a consumer can associate the reading with the exact local selector. Paths and vendor identity stay out of ordinary quota output. A discovered sibling lane is excluded from the refresh delegate, so when its credentials are rejected `state.reason` is `credentials_expired` and `--full` adds a `state.remedyCommand` (plus a matching `help` entry) naming that lane's own `CLAUDE_CONFIG_DIR` for a one-time `claude doctor`. Both carry the profile path, so both stay in the `--full` tier with `accountLocator`; ordinary output keeps the reason alone. Failed or identity-unverified accounts still have their local key; the key never asserts that authentication succeeded. The TUI displays the opaque key under each expanded card's provider name.
...
Account collection is shared in `src/providers/accounts.ts`. Adapters can implement `ProviderAdapter.discoverAccounts` with stable keys and bound quota/auth readers; collection preserves each account's success or failure. A discovery fault, or a discovery result with an invalid or duplicated key, degrades only that provider: it falls back to the single selected lane and names `account-discovery` in `state.degradedSources` (a `degraded_source` attention row, and an `account-discovery` source in the `auth` report) so a truncated account set is never silent. Claude is the first discovery implementation. Other adapters retain their existing source-selection behavior; additional Pi Codex entries and other providers' multi-account discovery remain future adapter work.
...
| `degraded_source`                                       | `all`   | A source was superseded: a credential source that was broken or unreadable while a sibling source answered, or `account-discovery` when enumerating a provider's accounts failed and only the single selected lane was read. `detail` is `<source> · <error>`. One row per source. |
...
| The Claude sibling-lane reauth `state.remedyCommand` and its `help` entry, which name that lane's config directory                    |

Everything a consumer branches on stays in the default tier: `state.status`, `stale`, `authStatus`, `error`, `reason`, `remedyCommand` (except the path-bearing Claude sibling-lane reauth command, which is demoted with `accountLocator` while its `reason` stays), `retryAfter`, `untrustedWindowIds`, and `degradedSources`; window `pace.status`, `reason`, `reservePercentPoints`, `burnMultiple`; `quotaSemantics.status` and `unresolvedWindowIds`; and every scope's `effectivePercentRemaining`, `boundedBy`, `limitingWindowIds`, `boundConflict`, `runway`, `selection`, and pace `aheadWindowIds` / `unknownWindowIds` / `worstReservePercentPoints`. `credits` also stays, so a consumer can avoid misreading it as exhaustion.

`--tui` renders from the complete in-memory model, so demotion never changes what the human report draws - except for the identity- and path-bearing fields above (`accountLocator` and the Claude sibling-lane reauth remedy), which `--tui` demotes the same way `--json` and TOON do so paths and vendor identity stay out of ordinary output. Every other field is unaffected by tiering.
...
| `degradedSources`    | Optional `{ source, error? }` list of credential sources a working sibling superseded on a fresh reading, plus `account-discovery` when account enumeration failed |
...
- Definitive credential absence or rejection retires Kimi cache data, but only when it is the verdict that speaks for the provider: a sibling source that failed transiently outranks it, so the cache survives, and a refreshable soft-expired verdict (`expired_refreshable`) is never definitive, so it also survives. What it retires is named per source: each source whose own credential failure was definitive loses the snapshot filed under the identity that source's reading would have carried, never whichever identity the run last published, and a source that failed only transiently keeps its snapshot. Transient fallback drops reset-expired windows and applies five-hour or seven-day age bounds to windows without resets.
