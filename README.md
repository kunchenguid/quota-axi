...
- It sends the short-lived access token only as a bearer to Kiro's first-party `AmazonCodeWhispererService.GetUsageLimits` endpoint, with a fixed read-only request body (only the optional profile ARN; account email is not requested) and no cookies. The response is normalized to the Kiro credit usage window, plan, balance, and reset time.
