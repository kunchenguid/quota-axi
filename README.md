...
A lone lane keeps the legacy keyless slot, which the single selected account uses too, so the snapshot itself records which ChatGPT account it came from (see [Cache](#cache)).
...
| Codex cache identities                 | Cached Codex windows are accepted only when ID, label, kind, duration, and duplicate suffix order agree; stale snapshots with mismatched identities are rejected. A snapshot also records an opaque SHA-256 identifier of the ChatGPT account the vendor attributed the reading to, and is withheld when the credentials a later run reads name only other accounts, because a slot name alone cannot say whose windows it holds. A snapshot without that identifier, or a run whose credentials name no account, proves nothing either way and is still reused.             |
