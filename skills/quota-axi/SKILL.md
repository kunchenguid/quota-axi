---
name: quota-axi
description: "Report local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Meta Muse Spark, Z.AI, Alibaba, OpenCode Go, Antigravity, and Command Code quota windows via the quota-axi CLI - remaining effective usable runway, percentages, reset times, cycle-average pace vs the reset clock, a per-scope selection signal, and provider status read from local auth sources, with no routing, no credential persistence, and no default ordering preference. Use before deciding whether it is safe to keep spending a provider's quota, when the user asks about usage, rate limits, pace, or remaining quota, or when comparing local provider headroom."
...
Report local Claude, Codex, Cursor, GitHub Copilot, Grok, Kimi, Meta Muse Spark, Z.AI, Alibaba, OpenCode Go, Antigravity, and Command Code quota windows.
quota-axi is data only: it never routes, recommends, ranks, or persists credentials. When the same stored
