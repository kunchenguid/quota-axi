...
const PI_PROVIDER_ID = "minimax-cn";
...
export type MinimaxCredentialInspection =
  | Exclude<MinimaxCredentialResolution["status"], "available">
  | "available";

export type MinimaxCredentialBroker = {
  resolve(): Promise<MinimaxCredentialResolution>;
  inspect(): Promise<MinimaxCredentialInspection>;
};
...
export function createPiMinimaxCredentialBroker(
  ...
  const inspect = async (): Promise<MinimaxCredentialInspection> =>
...
async function resolveCredential(
...
): Promise<MinimaxCredentialResolution> {
  ...
  // Pi stores a `minimax-cn` login as either a literal API key or the OAuth
  // record it received from MiniMax, mirroring its `kimi-coding` and `xai`
  // entries. Both are read in place: an expired OAuth record is reported as
  // expired rather than refreshed, because refreshing would mutate Pi's own
  // auth state, which quota-axi never does.
