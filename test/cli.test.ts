...
describe("endpoint throttling versus exhausted user quota", () => {
  it("represents an endpoint-throttled seat distinctly from an exhausted-quota seat", async () => {
    useTempCache();
    const [healthy, throttled, exhausted] = fiveSeatDirs();
    installClaudeSeatRouter({
      [basename(healthy)]: () => freshClaudeQuota(),
      // HTTP 429: the endpoint refused the read; capacity is unknown until retry.
      [basename(throttled)]: () =>
        failedClaudeSeat("rate_limited", "Claude quota endpoint rate limited", {
          retryAfter: "2026-07-24T19:30:00Z",
        }),
      // HTTP 200 with quota fully spent: a successful read, capacity known-empty.
      [basename(exhausted)]: () => exhaustedClaudeQuota(),
    });
    ...
    const throttledRow = bySeat[seatId(throttled)];
    const exhaustedRow = bySeat[seatId(exhausted)];

    // Endpoint throttling: no windows, only a retry time — do NOT dispatch, but
    // capacity is unknown, so this is not "quota spent".
    expect(throttledRow.state.status).toBe("rate_limited");
    expect(throttledRow.state.retryAfter).toBe("2026-07-24T19:30:00Z");
    expect(throttledRow.windows).toHaveLength(0);

    // Exhausted quota: a fresh read with zero remaining and a reset time — do
    // NOT dispatch, capacity is known-spent and there is no retryAfter.
    expect(exhaustedRow.state.status).toBe("fresh");
    expect(exhaustedRow.state.retryAfter).toBeUndefined();
    expect(exhaustedRow.windows[0]).toMatchObject({
      percentUsed: 100,
      percentRemaining: 0,
      resetsAt: "2026-07-27T00:00:00Z",
    });

    // The two failure modes never collapse into one another.
    expect(throttledRow.state.status).not.toBe(exhaustedRow.state.status);

    // Exhausted-but-readable data counts as usable; the throttled read does not.
    // A lone 429 therefore never forces exit 1 or erases the other seats.
    expect(bySeat[seatId(healthy)].state.status).toBe("fresh");
    expect(output.summary).toEqual({
      availability: "partial",
      ok: 2,
      unavailable: 1,
...
// A seat whose usage endpoint answered (HTTP 200) with the quota fully spent:
// a successful, fresh read that reports zero remaining and a reset time. The
// opposite of an endpoint-throttled (HTTP 429) seat, which has no windows.
function exhaustedClaudeQuota(): ProviderQuota {
  return {
    provider: "claude",
    label: "Claude",
    source: "oauth",
    ...
    state: {
      status: "fresh",
      stale: false,
      refreshedAt: "2026-07-20T18:10:00Z",
      sourcesTried: ["oauth"],
    },
    attempts: [{ source: "oauth", status: "success" }],
  };
